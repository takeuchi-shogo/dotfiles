...
**byte 数 proxy** の二段構え（最初は tiktoken 化しない。日本語 UTF-8 は 1 文字 3 byte のため、英文比で ~3 倍厳しい閾値になる点に注意）:
- **CLAUDE.md**: ≤ 4KB byte（推奨 3KB、英文 ~80 行 / 日本語 ~40 行）
- **AGENTS.md / .codex/AGENTS.md**: ≤ 4KB byte（英文 ~130 行）
- **references/ 単独**: ≤ 8KB byte（英文 ~200 行）
- 超過で **soft warning**（hard block しない）
- 日本語主体の doc で過大警告が多発する場合は tiktoken 化を検討
