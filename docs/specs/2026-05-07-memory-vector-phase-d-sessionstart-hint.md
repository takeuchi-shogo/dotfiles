...
acceptance_criteria:
  ...
  - top-K (K=5、distance < 1.0 の閾値あり) の path 一覧を additionalContext として stdout に出力する
  ...
  - hook 全体 latency が 300ms 以内 (model cold start 含む)
...
- 結果が distance threshold (1.0) を超える → 該当ファイルだけ除外、残りを提示
- 全件除外された → hint なし
...
- /Users/.../memory/codex_attention_depth.md (rel: 0.42)
- /Users/.../memory/feedback_codex_reasoning.md (rel: 0.38)
- /Users/.../memory/subagent_patterns.md (rel: 0.35)
...
- **distance threshold = 1.0**: spike の bench 結果 (Q4 で zero-relevance hit が出た) を踏まえた filter。1.0 は経験値、運用で調整
- **既存 SessionStart hook と並列**: 新規 hook 追加で既存挙動 (env-bootstrap、MEMORY.md auto-inject 等) は不変
- **同期 vs 非同期**: SessionStart は session 起動を待つ blocking event、ここで非同期化はできない。300ms 以内厳守
- **embedding model のロードコスト**: cold start ~150ms。SessionStart は session ごとに 1 回だけなので許容範囲
...
- `subprocess.run([node, query-script.ts, query], capture_output=True, timeout=2.0)` で semantic 取得
- 結果を「## R3 出力 format」に整形して stdout に出力
- exit 0 を保証 (例外時も log + 空出力)

### R2. Query script

- `.config/claude/scripts/runtime/memory-vec-query.ts` を新規追加
- spike.ts の `semanticTopK` 関数のみ抽出して書き出す
- argv[2] を query として受け取り、JSON 配列で stdout 出力:
  `[{"path": "...", "name": "...", "distance": 0.42}, ...]`
...
- type: `"command"`, command: `".config/claude/scripts/runtime/memory-vec-hint-hook.py"`
...
- **hook 性能**: 300ms 以内 (model cold start 含む)、超過したら hint skip
...
- 性能: `time .../memory-vec-hint-hook.py < /dev/null` で <300ms
...
- **K の変更**: hook script の `K = 5` を変更するだけ。format への影響なし
- **threshold の変更**: hook script の `THRESHOLD = 1.0` を変更するだけ
...
1. `.config/claude/scripts/runtime/memory-vec-hint-hook.py` を新規作成:
   ...
   - `subprocess.run([node, "memory-vec-query.ts", query], timeout=2.0)` で semantic top-5 取得
   - distance < 1.0 でフィルタし、Requirements R3 の format で stdout 出力
   - 例外時は ~/.claude/logs/memory-vec.log に追記、exit 0 で空出力
2. `.config/claude/scripts/runtime/memory-vec-query.ts` を新規作成:
