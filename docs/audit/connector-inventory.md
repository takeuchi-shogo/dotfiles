...
| `x-docs` | 1, 2, 3 | ✅ | ✅ | X (`https://docs.x.com/mcp`) | low | WebFetch | X API 公式 docs、認証不要 |
| `xapi` | 1, 3 | ❌ | ❌ (disabled) | X (`@xdevplatform/xurl` → `https://api.x.com/mcp`) | med | WebFetch + manual API | OAuth 2.0 要。`CLIENT_ID`/`CLIENT_SECRET` 設定後に enable |
...
## 検出済み drift (2026-06-30 実行時点)

`connector-drift-check.py` 実行で以下の drift を確認:

### 1 件: defined_but_not_enabled
- `xapi` — `.mcp.json` に定義されているが `enabledMcpjsonServers` に無い。
  意図的な未有効化 (OAuth 2.0 の `CLIENT_ID`/`CLIENT_SECRET` 設定が必要)。credentials 設定後に enable する。
...
- `code-review-graph` — ローカル graph DB 依存。Codex では Grep fallback

## 運用ルール

1. **drift の扱い**:
   - `defined_but_not_enabled`: **要解消** (enable か削除)。OAuth 等の前提条件がある場合は本表に理由を記載して allowed とする
...
- **2026-06-30**: X MCP 追加 (`x-docs` 有効化、`xapi` は OAuth 設定待ちで disabled)
