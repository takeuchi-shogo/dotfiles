...
## Task 5: /improve への pending 件数統合(入口3) — 不採用 (2026-06-01)

> **SKIPPED**: 実装時に `commands/improve.md` の設計境界と矛盾が判明したため不採用。
> improve.md line 121「読み取り専用のシグナル (autoevolve の learnings/) は参照しない (consumer 責務分離)」/ line 15「セッションデータ … 本コマンドからは参照しない」に正面衝突する。
> `/improve` は 2026-05-03 retire した autoevolve 版の反省から、あえて signals を断った薄いオーケストレーターとして再設計された経緯がある (line 127)。
> 入口は skill `/promote-learnings`(常時) + 週次 nudge の2本で十分機能するため、入口3は見送る。Research が improve.md を汎用チェックリストと仮定したことが原因 (プランの前提誤り)。
