...
現在の dotfiles スキル構成:
- 57 スキル中 **31 がフラット（SKILL.md のみ）**、20 がマルチ .md、6 のみリッチフォルダ
- リッチスキル率 **11%**（目標: enrichment archetype に基づき適切に構造化）

本設計は、5つの enrichment archetype を定義し、全57スキルを分類・優先度付けして段階的にリッチ化する。

### Archetype と Tier の関係

**archetype 分類は全スキルの型分け**（このスキルはどのパターンに当てはまるか）であり、**enrichment 対象かどうかは Tier で指定**する。archetype に該当しても既にリッチなスキルや Guard 型は enrichment 不要。
...
**該当**: edge-case-analysis, senior-frontend, senior-backend, senior-architect, graphql-expert, react-best-practices, vercel-composition-patterns, ui-ux-pro-max, buf-protobuf, web-design-guidelines, react-expert, dev-insights, dev-ops-setup
...
以下は archetype に明確に当てはまらないか、他スキルの補助的役割:
- **capture**: メモ取得（Generator に近い）
- **kanban**: タスク管理（Generator に近い）
- **developer-onboarding**: 開発者オンボーディング（Knowledge Base に近い）
...
## Implementation: Tier 3（12スキル, 12ファイル）
...
### 概ね完成（6スキル）
github-pr, graphql-expert, buf-protobuf, hook-debugger, search-first, init-project

### 低頻度・現状維持（5スキル）
dev-insights, dev-ops-setup, kanban, capture, developer-onboarding
...
新規追加ファイルは既存ファイルと**並存**する（置き換えない）:
- 既存テンプレート（例: `research-report-template.md`）はそのまま維持
- 新規ファイルは別の役割を担う（例: `subtask-prompt.md` はサブタスク投入用）
- 役割が重複する場合は既存ファイルを拡張し、新規作成しない

## SKILL.md への参照追加

新規ファイルを SKILL.md から発見可能にするため、**最小限の参照行を SKILL.md に追加**する。
これは「内容リライト」ではなく、以下のようなポインタ1-2行の追記に限定:

```markdown
## References
- 詳細な判断基準: `references/staleness-criteria.md`
- 出力フォーマット: `templates/health-report.md`
```
...
| コンテキストウィンドウ圧迫 | 応答品質低下 | 追加ファイルは on-demand 読み込み（SKILL.md からポインタで参照） |
| テンプレート形骸化 | メンテコスト増 | スキル呼び出し時にテンプレート参照を必須化 |

## Out of Scope

- 既存 SKILL.md の大規模リライト（参照ポインタの追記は scope 内）
