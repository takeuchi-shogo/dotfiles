...
revision_history:
  - 2026-05-24 initial — Phase 1: 13 採用 / 13 棄却
  - 2026-05-24 Phase 2 — ユーザーの /challenge 駆動で棄却 13 件を再評価、parity gap 残存 5 項目を追加採用 (#20 Mentoring Tone / #23 Author Preference Authority / #24 Documentation Sync / #10 LGTM with comments / navigate.md Navigation Order)。decision record: absorb-analysis.md §13
  - 2026-07-25 close — 実ファイル照合で T1a-T7 完了を確認、T8 は対象パスが repo 外と判明。Outcome 追記のうえ completed/ へ移動
---
...
Google エンジニアリング慣行 (eng-practices) から採用した 18 手法を AI review harness に統合し、コードレビューの品質・規律・一貫性を Google エンジニア水準に引き上げる。Phase 1 (13 採用) + Phase 2 (5 採用、/challenge 駆動の parity gap fix) で構成。

## Scope

- 採用項目: 18 件 (Phase 1: 13 件 [Gap/Partial 7 + Already 強化可能 6] + Phase 2: 5 件 [#20/#23/#24/#10/navigate.md])
- ユーザー意図: "Google エンジニアになりたい" 目線で mindset/discipline transfer を含めて全採用。AI review が Google 水準の規律を毎回適用する harness を構築する
- 棄却項目: 8 件 (#26/#22/#4/#5/#12/#19 + #7/#8/#9 N/A) ※ Phase 2 で #20/#23/#24/#10 を採用転換、navigate.md は Phase 1 で未抽出だった新規項目
- Phase 2 採用根拠 (decision record: `docs/research/2026-05-24-google-eng-practices-absorb-analysis.md` §13):
  - **#20 Mentoring Tone**: #5/#19 (context 説明) と #2 (evidence) は mentoring tone (Why + 学習リソース) を保証しない独立規律。AI reviewer の future-self education として transferable。Section G 実装
  - **#23 Author Preference Authority**: Principle 2 (Evidence-Based) は「根拠の品質」、Style Authority は「指摘範囲の制限」で論理的に直交。AI reviewer の主観 NIT 量産抑制に必要。SKILL.md Principle 4 実装
  - **#24 Documentation Sync** (本表記は CC-10): doc-gardener は既存 docs の定期 drift スキャン、本項目は本 CL 内同期強制。責務分離可能で並立。cross-cutting CC-10 実装
  - **#10 LGTM with comments** (再判定): FYI label は label レベル、Google LGTM = approve + non-blocking nits 並立の verdict 表現が欠落。`[NITS_REMAIN]` タグで operationalize。code-reviewer.md "Verdict 補助タグ" + SKILL.md Step 5 完了報告に実装
  - **navigate.md Navigation Order**: severity 順 (broad→specific→nit, Principle 3) と直交するファイル読み順 (main first → affected → trivial last)。SKILL.md Step 1.4 実装
...
### T2.5: Phase 2 採用転換 5 項目の統合 (/challenge 駆動)

**Items**: #20 + #23 + #24 (CC-10) + #10 + navigate.md (5 項目)
**Files**:
- `.config/claude/agents/code-reviewer.md` Section G (#20) + Verdict 補助タグ `[NITS_REMAIN]` (#10、※ 補助タグ自体は別ファイル参照のみ。本 Plan 改訂時点では doc 未追記、要 follow-up)
- `.config/claude/references/review-checklists/cross-cutting.md` CC-10 (#24)
- `.config/claude/skills/review/SKILL.md` Principle 4 (#23) + Step 1.4 Navigation Order (navigate.md) + Step 5 完了報告 `[NITS_REMAIN]` 出力 (#10)
**Scale**: M (累計 +135 行: code-reviewer +19 / cross-cutting +29 / SKILL.md +59 / absorb-analysis §13-14 +47 / 本 Plan 改訂)
**Status**: 実装済 (本 Plan 改訂と同 CL 内、絶対整合は absorb-analysis.md §13 を正本)

**Rationale (Phase 2 採用転換)**:
- Phase 1 完了直後、ユーザーが `/challenge` で「Google エンジニア基準で同等以上達成?」を独立検証
- 棄却 13 件のうち 5 項目に parity gap 残存と判明、ユーザー承認 ("全部進めて") で Phase 2 として追加採用
- decision record の正本: `docs/research/2026-05-24-google-eng-practices-absorb-analysis.md` §13 (経緯) + §14 (Meta Finding: /absorb workflow 改善示唆)

**Acceptance criteria**:
- 5 項目すべてが対応ファイルに実装されていること (Section G / Principle 4 / Step 1.4 / CC-10 / `[NITS_REMAIN]`)
- absorb-analysis.md §13 の表 (棄却決定を覆す根拠) と本 Plan の Phase 2 採用根拠が整合
- 既存 Section A-F → G、CC-1〜9 → CC-10、Principle 1-3 → 4、Step 1.3 → 1.4 の番号体系が連続
- /review BLOCK 解消 (本セクション追記で Plan-Implementation alignment 回復)

**Follow-up (本 CL 後の TODO)**:
- `agents/code-reviewer.md` に "Verdict 補助タグ" セクション (`[NITS_REMAIN]` の verdict 計算上の扱い) を追記 → 別 CL
- SKILL.md Step 1.4 で言及される Pass 1 (`agents/code-reviewer.md` Step 2) の参照整合確認 → 別 CL
...
- 外部知見索引セクションに以下を追記:
  ```
  - [Google eng-practices (2026-05-24)](../../../dotfiles/docs/research/2026-05-24-google-eng-practices-absorb-analysis.md) — 18 手法採用 (Phase 1: PR 分割 5 patterns / courtesy Bad-Good 例文 / emergency 定義 / threshold 300 行統一 / positive principle / evidence-based feedback / design-first gate + Phase 2: mentoring tone Section G / author preference Principle 4 / documentation sync CC-10 / LGTM-with-comments [NITS_REMAIN] / navigation order Step 1.4)。新規 3 ファイル + 既存 8 ファイル編集。
  ```
...
全タスク完了後:
- 18 採用項目すべてが対応ファイルに反映されていること (Phase 1: 13 + Phase 2: 5、逆引きチェックリストを T8 実施前に作成)
...
- 18 採用項目 (Phase 1: #14/#21/#25/#1/#2/#6/#13/#15/#16/#11/#3/#17/#18 + Phase 2: #20/#23/#24[CC-10]/#10/navigate.md) すべてが対応ファイルに反映されていること
...
## Phase 2 (2026-05-24 同日追記): /challenge による棄却決定の見直し

Phase 1 完了直後、`/challenge` skill で「Google エンジニア基準で同等以上か」を独立検証した結果、棄却見送り 13 件のうち 5 件に parity gap が発見されたため Phase 2 として追加採用。詳細根拠は `docs/research/2026-05-24-google-eng-practices-absorb-analysis.md` §13 参照。

### Phase 2 採用項目

| # | 手法 | 棄却決定を覆す根拠 (要約) |
|---|------|------------------------|
| #20 | Mentoring Tone | #5/#19/#2 は context/evidence を保証するが mentoring tone (Why + 学習リソース) は別軸 |
| #23 | Author Preference Authority | Principle 2 (根拠品質) と Principle 4 (指摘範囲) は論理的に直交 |
| #24 | Documentation Sync | doc-gardener = 既存 drift スキャン / CC-10 = 本 CL 内同期、責務分離可能 |
| #10 | LGTM with comments | FYI label は label レベル、Verdict 並立表現 `[NITS_REMAIN]` で operationalize |
| navigate.md | Navigation Order | severity 順 (Principle 3) と直交する file 読み順、Phase 1 未抽出だった追加発見 |

### Phase 2 タスク

| Task | 対象ファイル | 対応手法 | 規模 |
|------|------------|---------|------|
| T9 | `agents/code-reviewer.md` Section G | #20 | S |
| T10 | `skills/review/SKILL.md` Principle 4 | #23 | S |
| T11 | `references/review-checklists/cross-cutting.md` CC-10 | #24 | S |
| T12 | `agents/code-reviewer.md` Verdict 補助タグ + `skills/review/SKILL.md` Step 5 完了報告 | #10 | S |
| T13 | `skills/review/SKILL.md` Step 1.4 Navigation Order | navigate.md | S |

### Phase 2 Acceptance criteria

- 5 追加項目 (#20/#23/#24/#10/navigate.md) が対応ファイルに反映されていること
- Phase 1 の 13 採用項目と矛盾しない (Principle 2/3/4 が論理的に AND で適用可能、Section A-G が独立参照可能)
- code-reviewer + codex-reviewer 並列 review で PASS verdict 取得 (Plan Alignment Contradiction が解消されていること)

### Phase 2 規模

- 3 ファイル編集 (`code-reviewer.md` / `cross-cutting.md` / `review/SKILL.md`) + 2 ファイル追記 (本 plan + absorb-analysis.md)
- 総追加行数: 約 120 行 (Phase 1 の 11% 程度の増分)

---

Generated by /absorb on 2026-05-24 (Phase 1)、/challenge + manual edits on 2026-05-24 (Phase 2)

---

## Outcome (2026-07-25)

実装は `d86cf96e` (Phase 1: 13 disciplines) と `2ad58996` (2026-05-26 deep dive gaps) の 2 コミットで入っていた。plan 側に進捗記録がなかったため、実ファイル照合で完了状態を確定した。

| Task | 状態 | 実在確認 |
|------|------|---------|
| T1a / T1b / T1c | 完了 | `references/{pr-splitting-patterns,review-courtesy-examples,emergency-definition}.md` |
| T2 | 完了 | `agents/code-reviewer.md` Section A–E |
| T2.5 (T9-T13) | 完了 | Section G / Principle 4 / Step 1.4 / CC-10 / `[NITS_REMAIN]`。plan 記載の follow-up 2 件 (Verdict 補助タグ / Step 1.4 の Pass 1 参照整合) も実装済み |
| T3 | 完了 | `skills/github-pr/SKILL.md` Size & Splitting (threshold + Large CL Exception + 分割パターン参照) |
| T4 / T5 | 完了 | `skills/github-pr/self-review.md` / `agents/golang-reviewer.md` とも 300 行で統一 |
| T6 | 完了 | `references/review-checklists/cross-cutting.md` CC-4 (新規 TODO / 既存 TODO 区別) |
| T7 | 完了 | `skills/review/SKILL.md` Principle 1–3 |
| T8 | 対象外 | 下記参照 |

### 未解決事項

**T8 のファイルパスが誤っていた。** plan は `.config/claude/projects/-Users-takeuchishougo-dotfiles/memory/MEMORY.md` を編集対象としていたが、このパスは repo に存在しない。MEMORY.md は `~/.claude/projects/` 配下にのみ実在し git 管理外で、dotfiles の PR では触れない。MEMORY.md へのポインタ追記は repo 外の作業として別途実施する。

### Threshold の再評価 (本 close と同 CL)

merged PR 直近 40 件を実測した結果、中央値 55 行 / 100 行以下が 60% / 300 行超が 20% で、300 行の分割検討ラインは 80 パーセンタイル付近に位置していた。一方で Google の "1000 lines is too large" / "50 files is spread too many" に対応するゲートが無く、1000 行超の PR が 5 件 (12.5%、最大 2724 行)、50 ファイル前後が 2 件通過していた。

- 300 行 (分割検討必須) は据え置き。T4/T5 の同期ファイルは変更不要
- 推奨サイズとして 100 行を明記 (ゲートではなく目標)
- 1000 行 / 50 ファイルを上限として追加。emergency 認定時のみ例外
