...
- [x] 既存 session_observer の出力フォーマットが壊れていない (snapshot test) — TestExistingFormatPreserved 3 tests pass
- [x] cache_read=0, cache_create=0 のセッション (新規 cold start) で divide-by-zero しない — test_cold_start_returns_none pass
- [x] ratio = 80%+ なら warning なし、< 30% かつ連続 cache_create > 3 で warning フィールド出力 — test_high_ratio_above_80_never_warns + test_warning_fires_on_4th_consecutive_miss + test_streak_exactly_at_limit_no_warning pass
- [x] resource-bounds.md に新規閾値が記載され、`task validate-configs` が通る — exit 0 確認済 (2026-05-22)
