...
- **idle の扱い**: 記事は「10 分以上 idle なら通知」を推奨するが、dotfiles では idle =
  ターンが終わって待機中であり異常ではない。idle 単独では**通知しない**。
  ただし **idle かつ dispatch ログ上は未回収** は異常である (worker は仕事を終えたのに
  誰も結果を取りに来ていない状態)。この 2 つを混同すると、記事 v0.8.0 が踏んだ
  「全員 idle・誰も失敗していないのに全体が止まる」を検出できない。
  未回収の判定は `dispatch-log.sh pending` に委ねる (2026-08-03 実装、全セッション横断)。
  発火条件は `agent_status == idle/done` ∧ `pending に載っている` の AND。
...
- **上 2 つは合成テストであり、これだけで完了としない**。実際に
  `herdr-launch-worker.sh` → 意図的に `herdr-collect-result.sh` を呼ばない、という
  委譲サイクルを 1 本通し、通知が出ることを確認する。記事 v0.8.0 は合成テスト 11 項目が
  全通過した状態で実作業 1 サイクル目に停止しており、この差分こそが検出対象。
