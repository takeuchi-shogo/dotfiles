...
- [x] Phase 1: read-only MVP (Task 1.1-1.4) — **2026-06-04 実装完了** (`/rpi`)。
  ...
  検証: collect/ledger/aggregate/lock/timeout ガードは DRY_RUN・合成テスト・pytest(8)で実証。**live claude 成功走行のみ保留** (claude -p の一時劣化でブロック、外部要因)。
  **未完了の運用ステップ**: `bash scripts/runtime/nightly/launchd-install.sh` で実 launchd 登録 (実 load を伴うためユーザー実行待ち)。
- [ ] R1 判定 (30日運用後)
...
- **残作業**: (1) `launchd-install.sh` 実行で実 launchd 登録、(2) claude -p 回復後の live 成功走行 1 回確認、(3) コードレビューゲート (claude/codex 劣化時は回復後)。
- 30日 ledger 蓄積後に R1 判定 → Phase 2 へ。
