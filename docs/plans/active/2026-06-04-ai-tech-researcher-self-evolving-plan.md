...
- [ ] R1 判定 (30日運用後)
- [x] Phase 2: source ledger + 多軸選別 — **2026-06-06 PR #66 でマージ済** (`feat/tech-researcher-phase2`)。
  実装:
  `sources.py` (sources-ledger 管理: bootstrap/list-active/refresh/prune, pure logic+CLI) +
  `test_sources.py` (新規), `aggregate.py` (--by-source/scores 平均 clamp/2状態区別),
  `run-tech-researcher.sh` (sources-ledger 連携・多軸 JSON-lines parse・ledger v2 (source_id+scores)・refresh)。
  forward-compat 4点 (source_id/type:web=feed URL/retention=prune helper/2状態) 全処理。
  検証: pytest 64 green + bash -n + 隔離 DRY_RUN E2E (本番非汚染、5 sources/22行/by-source 出力実証)。
  レビューゲート: Claude 6体パネル + security focused 再レビューで NEEDS_FIX→修正6件 (id整数検証/SSRF
  canonicalization+inet_aton短縮形/0採用vs block欠落区別/list-active stderr/asof検証/keyword score skip)。
  **Codex Review Gate は Bash tool 経由到達不能のため未実施 (回復後に別途)**。
  **Phase 3 前提 (security 由来)**: DNS-rebinding/fetch-time pin は未対応 (Phase 3 で external URL を
  is_safe_feed_url に通す前に検討)。設計 plan: `tmp/plans/dynamic-churning-hoare.md`。
- [ ] Phase 3: 全自動進化 (MAB/MMR/減衰 + auto-apply + sanity gate、human 承認なし)
- [ ] Phase 4: drift 自己監視
- [~] Phase 5: absorb-candidate bridge (harness 改善還元、人間ゲート維持) — **Issue #100 で「surface only → 分析レポート PR 化」に格上げ**。Stage A/B に分割:
  - [x] **Task 5.1 (harness_relevance 軸)** — 2026-06-15 実装 (`feat/tr-harness-relevance`)。judge prompt を4軸化 (novelty/reliability/concreteness + **harness_relevance**)、ledger v2 scores に carry (旧3軸行=null 後方互換)、`trends_select.select_harness_candidates()` で閾値選択土台を追加。検証: pytest 89 green + bash 4軸 parse 狙い撃ち + 隔離 curl-shim E2E (ledger に harness_relevance 着弾実証)。aggregate.py は非変更 (AXES はソース進化スコア用で absorb 価値とは別概念=関心分離)。
  - [ ] **Task 5.2 (閾値超えの晩のみ分析 PR を無人化)** — Stage B。cold-start のためスコア蓄積後に着手 (閾値選択はデータが要る)。未決事項: 閾値・worktree 隔離 git・生成ツール・absorb-candidate dedup ledger・分岐名/PR body の injection sanitize・graceful degrade・**同一 URL が複数日採用された際の dedup key** (現状 `select_harness_candidates` は `select` 継承で最新日採用 → 最高スコア採用にするか要決定、review FYI)。
- [ ] R3/R4/R5 判定
