...
| **C** | `system.defaults` 統合 (低〜中 risk domain) | `2026-04-26-nix-migration-phase-c-plan.md` | NSGlobalDomain / dock / finder / trackpad / a11y / menuextra / screensaver / controlcenter が宣言化、private プロファイルで `darwin-rebuild` 一発再現 |
| **D** | 致命 risk domain (HIToolbox / symbolichotkeys) | (C 完了後に作成、spike 先行) | 入力ソースとシステムショートカットが宣言化、入力不能・操作不能の rollback 手順実証済み |
| **E+** | アプリ固有設定 (Safari / Terminal / etc.) | (D 完了後に必要分のみ作成) | 個別 app 単位で plan、必要なら何 phase にもなる (E1, E2, ...) |
| **F** | `com.apple.spaces` 等 nix-darwin module 未対応 domain | (要事前調査、対応していなければ activationScripts 経由) | spaces 設定が宣言化、または manual scope として明文化 |
| **W** | work プロファイル実機検証 | (新品 work Mac 入手後) | `darwin-rebuild switch --flake ./nix#work` 一発で 0→100% セットアップ |
...
- [x] Phase 0+A: Bootstrap (completed 2026-04-24)
- [x] Phase B1: Brewfile migration (completed 2026-04-25)
- [x] Phase B2: symlink.sh migration (completed 2026-04-26)
- [ ] Phase C: system.defaults 低〜中 risk domain (plan: active/2026-04-26-nix-migration-phase-c-plan.md)
  - [x] C0: 復旧 e2e テスト (2026-04-27、Tier 1 では defaults 値が残ること実証 → Tier 2 必須化)
- [ ] Phase D: HIToolbox / symbolichotkeys (致命 risk)
- [ ] Phase E+: app 固有設定
- [ ] Phase F: spaces 等 nix-darwin module 未対応 domain
- [ ] Phase W: work プロファイル実機検証

## Surprises & Discoveries

- **2026-04-27 Phase C0**: `darwin-rebuild --rollback` は generation を前世代に戻すが、`defaults write` で書き込み済みの値は OS plist 上に残存する (silent partial state)。Tier 2 (`defaults import` + `killall cfprefsd`) で初めて plist 単位の正確な復旧が可能。C1 以降の milestone は **Tier 1 + Tier 2 のセットを必須化**。詳細: `2026-04-26-nix-migration-phase-c-plan.md` の Surprises & Discoveries
