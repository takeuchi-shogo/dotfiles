...
| Q | 確定内容 |
|---|----------|
| Q1: friction-events path | `~/.claude/agent-memory/learnings/friction-events.jsonl` (`find` で確認済) |
| Q2: skill-audit 呼び方 | 既存 `/skill-audit` skill を `claude -p "/skill-audit"` で呼び出す (DRY、独立実装はしない) |
| Q3: dep-audit 対象 | dotfiles root のみ: `~/dotfiles/{package.json,pyproject.toml,go.mod,Cargo.toml}` の存在するもの。サブディレクトリの package.json は v1 では除外 (YAGNI、将来 spec で拡張) |
| Q4: 06-Nightly retention | v1 は無制限蓄積。`ls 06-Nightly | wc -l > 100` で警告するだけの cleanup script は別 plan |
| Q5: morning-briefing 統合 | post-processing 方式 — `auto-morning-briefing.sh` の末尾近くで `prepend_nightly_status()` 関数を呼び、briefing 文字列の先頭に `## Nightly Status` セクションを差し込んでから Daily Note に append。PROMPT 改変はしない (既存 briefing 生成ロジックは無変更) |
| Q6: status API | positional 引数: `status_begin "$TASK"` / `status_end "ok\|fail" "msg" "metric_key=value..."` |
...
- [ ] **Step 1: Q1-Q7 を Pre-flight テーブル通りに確定し、本 plan の Pre-flight 表に追記済を確認**

(本 plan の冒頭 "Pre-flight" テーブルが正本。Q1-Q7 全てに確定値あり。再確認のみ)

- [ ] **Step 2: Pre-mortem チェック (references/pre-mortem-checklist.md 適用)**
...
- (FM-4) catch-up が誤発火 (`last_run` ファイル破損) → 破損時は `1970-01-01` fallback で必ず gate 通過、`mark_run_today` は idempotent
- (FM-5) claude -p が hang して翌朝まで起動しっぱなし → `timeout 900s` 必須、Task 5 template に組み込み

- [ ] **Step 3: Reversible-decisions チェック (references/reversible-decisions.md 適用)**
...
- [ ] **Step 4: Codex Spec/Plan Gate を起動 (cmux Worker 経由)**

```bash
~/dotfiles/scripts/runtime/launch-worker.sh --model codex --task "次の plan + spec の Spec/Plan Gate review を依頼: docs/superpowers/specs/2026-05-25-nightly-vault-automation-design.md と docs/plans/active/2026-05-25-nightly-vault-automation-plan.md を読み、(1) spec の抜け漏れ (2) plan の妥当性 (3) 潜在リスク (FM-1..5 以外で見落としあるか) を統合レビューしてほしい。"
```

cmux 不在 fallback:
```bash
codex exec --skip-git-repo-check -m gpt-5.5 --sandbox read-only --config model_reasoning_effort="xhigh" "<同上 prompt>"
```

期待: Codex の review コメント。critical な指摘あれば本 plan を修正してから Task 1 に進む。

- [ ] **Step 5: Codex 結果を反映 + Pre-flight テーブル固定をコミット**
