...
`claudemd-size-check.py` (PostToolUse hook) が次の**いずれか一方でも** warn したとき
(byte と行数は独立に判定される):
- `MEMORY.md byte budget exceeded: >23000B` (25KB の 92%)
- `MEMORY.md line budget exceeded: >180 lines` (200行の 90%)

(Claude Code ハード上限 200行/25KB の手前で proactive に警告)
...
```bash
# <project-slug> は対象プロジェクト絶対パスの区切りを - に変換した形式
# 例: ~/dotfiles → ~/.claude/projects/-Users-<user>-dotfiles/memory/
