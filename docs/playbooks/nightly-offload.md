...
| `friction-aggregate` | 毎晩 23:20 (DAILY) | 前日 friction events 集計 |
...
- `friction`: total + class breakdown top 5
...
`crontab -e` で以下の 7 行を追記。**先頭の env block は必須** (Codex Gate C1/Q10 反映、cron は `.zshenv` を読まない):
