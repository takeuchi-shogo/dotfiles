...
3. severity prefix (`[BLOCKING]` / `[MAJOR]` / `[MINOR]` / `[NIT]`) は body 冒頭に残す
...
```bash
codex exec review --base "{{BASE_BRANCH}}" "PR #{{PR_NUMBER}} を深掘りレビュー。観点①② で見落としそうな structural issue / business logic risk / breaking change / 同 flow への波及を指摘してください。深い推論が必要な箇所に集中。⚠️ 指摘は diff 行に関連するものに限定。diff が他を破壊している場合は波及先も対象、ただし無関係な領域へのリファクタ提案は禁止。⚠️ 著者が PR body で明示した技術判断 (例: 3 env 共通鍵、特定ライブラリ採用、migration 戦略) と矛盾する代替設計を BLOCKING 根拠としない。代替案は FYI / SHOULD に留める。著者判断は事前に Phase 0.5 で抽出済み (prompt に含む想定)。 ⚠️ 実装方針への指摘は KW 既存実装 (\`product/**\`) に同パターンが無いか確認してから出す。確立パターンなら FYI 止まり。ただしバグ / breaking / セキュリティはパターン一致でも指摘 (override)。"
```
