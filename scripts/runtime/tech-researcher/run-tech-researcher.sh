...
PROMPT="あなたは技術トレンドのキュレーターです。下記の候補記事から、(a) AI 技術トレンドとして重要・新規性が高い、または (b) ソフトウェアエンジニアリングの良質な知見 (言語/設計/アーキテクチャ/ツール等、後で参照する「引き出し」になる) もの、を選別し、日本語の日次レポートを生成してください。
