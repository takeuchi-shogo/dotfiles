...
## Security Analysis
- セキュリティ深掘り調査は xhigh + read-only で行う: `codex exec --skip-git-repo-check -m gpt-5.6-terra --sandbox read-only --config model_reasoning_effort="xhigh" "..."`（`-p security` は v0.144.6 でエラーになる。理由: `.config/claude/rules/codex-delegation.md`）
...
## Subagent Usage
- `spawn_agent` は `task_name` / `message` / `fork_turns` のみを取る built-in の仕組み。名前付き custom agent の選択はできない
- `max_threads = 4`、`max_depth = 1` で並列委譲は引き続き有効
...
| `multi_agent` | `max_threads=4`, `max_depth=1` で built-in subagent を並列委譲 |
