...
- 外部状態を変えた後・handoff・障害報告では「次に人間が開く場所」をクリック 1 回で辿れる形で示す（同ファイルの「リンクの示し方」。secrets 設定画面の URL は書かない）
...
- **自律的バグ解決 + 3点説明**: 生データ（ログ・スタックトレース・CI出力）を直接分析。修正時は原因・修正内容・効果を必ず明示。報告時はエラー/ログの最短の決定的 1 行を verbatim 引用し全文を dump しない（解析は全文、提示は決定行）
- **ドキュメント＝インフラ**: 仕様書は耐荷重構造物。「2 回説明したら書き下ろせ」。spec/reference に codify する
- **指示の重複を作らない (instruction DRY)**: CLAUDE.md / AGENTS.md / memory / skill に新ルールを足す前に、既存が同じか矛盾を持たないか照合する。重複していれば parallel を新設せず、削除・統合・scope と優先度を明記した書き換えで一本化する
