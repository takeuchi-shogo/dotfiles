...
- デバッグ: `debugger` に委譲、または cmux Worker (`launch-worker.sh --model codex`) で深い分析 → 根本原因を受け取る
...
**トレードオフ**: 相談のたびにコンテキスト消費が増える。unknown unknowns（Executor が困難を認識できない場合）には hook ベースの検出（`claude-hooks` post-bash のエラー検出 等）で補完する。
...
| 決定的 CLI (`npm audit` / `tsc` / formatter / scanner) をラップする subagent を作る | CLI を直接実行する。結果の解釈だけが必要なら親 or 既存 agent に渡す（20K 起動コストを CLI 0.4-2s に払わない） |
...
| **select** | 固定スーパーグラフからサブグラフを選択 | タスクは似ているが難易度やツールが異なる | claude-hooks (Rust, user-prompt) が規模で reviewer 数を選択 |
| **generate** | 実行前にタスク固有のグラフを生成 | タスク種類自体が異なり、異なる構造が必要 | `/autonomous` が DAG を生成し worktree 並列 |
| **edit** | 実行中にグラフ構造を変更 | 部分実行で予期せぬ情報が現れ構造変更が必要 | claude-hooks post-bash がエラー時にルート変更 |
