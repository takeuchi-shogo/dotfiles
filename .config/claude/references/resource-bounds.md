...
`/fast` は Opus を高速出力モードに切り替える（**モデルのダウングレードではない**。Opus 4.8/4.7/4.6 で利用可）。speed > depth のタスクで使い、推論品質が支配的なタスクでは standard に戻す。
...
注: `overage-disabled` ヘッダ（上記 429 表）で fast mode は恒久停止され、standard に自動 fallback する。
