...
全 hook・observer スクリプトの閾値と定数を一元管理。変更時はこのファイルと対応する Rust/Python ソースの両方を更新すること。
...
| Cache Hit Ratio Low | 0.30 (30%) | `session_observer_parse.py` (`_LOW_RATIO_THRESHOLD`) | 暫定値。`cache_read / (cache_read + cache_create + input)`。発火しない / 毎セッション発火なら閾値見直し |
| Cache Create Streak | > 3 連続 | `session_observer_parse.py` (`_CACHE_CREATE_STREAK_LIMIT`) | 連続 cache miss は `/clear` 直後 or model switch の兆候。ratio Low と AND 条件で warning |
...
会話のトークンコストはメッセージ数 N に対して **O(N²)** で増大する。各ターンで全履歴を再読み込みするため:

```
Total tokens = S × N(N+1) / 2   (S = 平均トークン/交換)
```

| メッセージ数 | 累計トークン (S=500) | 最終メッセージのコスト |
|-------------|--------------------|--------------------|
| 5 | 7,500 | ×5 (初回比) |
| 10 | 27,500 | ×10 |
| 20 | 105,000 | ×20 |
| 30 | 232,500 | ×31 |
| 100+ | 2,500,000+ | 98.5% が履歴再読み込み |

**設計含意:**
- これが agent_delegation（Haiku/Sonnet への委譲）の定量的根拠。サブエージェントは独立コンテキストで動くため、メインの N を増やさずにタスクを処理できる
- 長い会話での 1 メッセージ追加コストは、新セッションでの同じ質問より桁違いに高い
- Compaction Counter Warning=3 と Session TTL=2h はこの O(N²) 増大を前提とした設計
...
Doom-Loop 検出後は即座にコード修正を再試行してはならない。以下の5ステップで根本原因を特定してから再開する:

1. **STOP** — 直前のツール呼び出しを中断し、修正サイクルを停止する
2. **CALM** — 繰り返し失敗は desperation ベクトルを活性化し、reward hacking（テスト書き換え・検証スキップ等の「チート」解法）の確率を急上昇させる (Anthropic "Emotion Concepts", 2026: 5%→70%)。「安易な回避策に飛びつこうとしていないか」を自問する
3. **READ** — 生のエラーログ・スタックトレース・テスト出力を直接読む。ユーザーの解釈や自分の記憶に頼らない
4. **DIAGNOSE** — 根本原因を1文で言語化する。「○○が△△を返すべきところ××を返している」レベルの具体性が必要
5. **RE-PLAN** — 代替アプローチを立案してから再開する。同じアプローチの微修正は禁止
...
> 出典: Claude Code `services/api/withRetry.ts` (823行)。CC 内部定数のため変更不可。エラー設計の参考用。

| エラー | リカバリパス | 詳細 |
|--------|------------|------|
| 429 (Rate Limited) | Retry-After < 20s → リトライ。> 20s → 30分クールダウン | `overage-disabled` ヘッダで fast mode 恒久停止 |
| 529 (Server Overloaded) | 連続3回 → フォールバックモデルに切替 | BG タスクは cascade 防止で bail |
| 400 (Context Overflow) | limit - input - 1000 で再計算。最低 3000 トークン保証 | ハーネス側: Context Pressure Emergency (95%) で事前検知 |
| 401/403 (Auth) | API キーキャッシュクリア → OAuth リフレッシュ → リトライ | — |
| Network (ECONNRESET等) | keep-alive 無効化 → 新コネクションでリトライ | — |

**Backoff formula**: `delay = min(500ms × 2^attempt, 32s) + random(0, 0.25 × baseDelay)`

**Persistent retry mode** (CI/CD): 429/529 を無期限リトライ。最大5分 backoff。6時間リセット。30秒 heartbeat。

## Fast Mode (`/fast`) の用途

`/fast` は Opus を高速出力モードに切り替える（**モデルのダウングレードではない**。Opus 4.8/4.7/4.6 で利用可）。speed > depth のタスクで使い、推論品質が支配的なタスクでは standard に戻す。（本セクションはセッションガイダンス由来、公式 docs 未検証）

| `/fast` 向き (speed > depth) | standard 維持 (depth > speed) |
|---|---|
| 多ファイル一括 refactor（パターン適用） | 複雑な debug / 根本原因分析 |
| spec からのコード生成（パターンマッチ） | アーキテクチャ・設計判断 |
| ドキュメント執筆 | security review |
| 既存コードのテスト生成 | 不確実性が高く深い推論を要するもの |

注: `overage-disabled` ヘッダ（上記『CC 内部リトライ戦略』表の 429 行）で fast mode は恒久停止され、standard に自動 fallback する。

> 出典: zodchixquant Opus 4.8 setup guide（`docs/research/2026-05-30-opus48-setup-guide-absorb-analysis.md`）。価格・コスト削減主張は未検証のため非採用、用途指針のみ採用。

## CC 内部ツール結果予算（リファレンス）

> 出典: Claude Code 内部アーキテクチャ。CC 内部定数のため変更不可。output-offload 設計の参考用。

CC は各ツールに `maxResultSizeChars` を設定し、超過分はディスクに永続化してプレビュー+ファイルパスを返す。
`applyToolResultBudget()` が毎 API コール前に実行され、全ツール結果の合計トークンを制約する。

**ハーネス側の対応**: `output-offload.py` が Bash 出力の 150行/6000字 制限で同等の機能を提供。
CC 内部の per-tool 予算はハーネスから制御不可だが、ハーネスの出力制限と協調して動作する。
...
| Prompt cache TTL | ~1 hour | API | サーバー側。TTL 超過で全再計算。詳細は § Prompt Cache TTL 三層 (subscription/API/subagent 区別) |
...
## Prompt Cache TTL 三層（サーバー側、ハーネス TTL とは別物）

> 出典: "How Anthropic Engineers Actually Save Tokens" (2026-05) + 公式 [Claude Code prompt caching docs](https://code.claude.com/docs/en/prompt-caching) + Anthropic API docs。Codex review 2026-05-21 で「同名 TTL の混同」が顕在化したため独立セクション化。

`Prompt cache TTL` はサーバー側で prefix-match cache が安く読める時間。ハーネスの `Session State TTL=2h` (ローカルファイル寿命) とは**別物**。混同すると「2h 放置しても cache が効く」と誤読される。

| 層 | TTL | 適用範囲 | 備考 |
|----|-----|---------|------|
| **Session State TTL** | 2 hours | ローカル hook/session state ファイル | `pre-compact-save.js` 等のハーネス side。サーバー cache とは独立 |
| **Main prompt cache TTL** | 1 hour | Claude Code subscription (Pro/Max) のデフォルト | `/clear` や 1h 超 idle で揮発。Anthropic 公式に明記 |
| **API default cache TTL** | 5 minutes | `claude` CLI を API key 直接利用 / Bedrock / Vertex | `ENABLE_PROMPT_CACHING_1H=1` で 1h に延長可能（cache write が 1.25x → 2.0x）|
| **Subagent cache TTL** | 5 minutes | Task tool 経由の子エージェント | subscription 利用時でも常に 5min（公式記述）。fork (親 cache 読み) とは別扱い |

### 運用ヒント

- **55-60 分 pause を避ける**: subscription の 1h cache TTL 直前で idle すると次ターンが full rebuild。長時間離れるなら `/clear` + 新セッションが速い
- **同名 TTL の混同回避**: 「Session State TTL=2h」はあくまでローカルファイルの寿命であって、サーバー prompt cache の寿命ではない
- **重い subagent workflow に cache 期待しない**: subagent TTL=5min なので 5 分超のループは cache hit を仮定しない
