...
## Shared Normative Constraints

> MemCollab (arXiv:2603.23234): モデル別に整理されたメモリを他モデルに直接転送すると性能が低下する
> （エージェント固有バイアスが混入するため）。対比蒸留で抽出したモデル非依存の制約は全エージェントに有効。

**このセクションには、クロスモデル対比から蒸留されたエージェント非依存の規範的制約を蓄積する。**
形式: `enforce: {守るべき不変条件}; avoid: {対応する失敗パターン}`
上の Model Insights セクションが「誰が発見したか」を記録するのに対し、ここは「何が転用可能か」を記録する。

| date | domain | enforce | avoid | source |
|------|--------|---------|-------|--------|
| — | — | (対比蒸留の実運用開始後に追記) | — | — |
...
|------|--------|------------|
| 2026-03-26 | memory-transfer | **Agent-Specific Bias Entanglement**: あるモデルのメモリ/経験を別モデルに直接転送すると、スタイル的嗜好やヒューリスティックショートカットが混入し性能が低下する。MemCollab Fig 1: Qwen32B のメモリを 7B に転送すると baseline 以下 (50.6% < 52.2%)。対策: モデル固有バイアスを除去した規範的制約（enforce/avoid 対形式）として蒸留してから共有する |
