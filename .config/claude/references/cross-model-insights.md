...
### Verbalized Confidence Inflation（構造的不変量）

> Zhao et al. "Wired for Overconfidence" (arXiv:2604.01457, 2026-04):
> 信頼度の言語化回路 (Confidence Mover Circuit) と事実検索回路は**構造的に分離**。
> CMC は中〜後段レイヤーに集中し（≥80% cross-dataset 共通）、
> 事実性回路（前段〜中段の長経路）との不一致がデフォルト高信頼バイアスを生む。
> RLHF 等の post-training がこのバイアスをさらに増幅する。

- **前提**: LLM が言語化する信頼度は、内部の不確実性を忠実に中継していない
- **含意**: レビューアーの「confidence: 95%」は体系的に膨張した数値
- **対策**: `evaluator-calibration-guide.md` の Verbalized Confidence Discount 原則を参照
- Typicality Bias と同様、クロスモデル検証でも過剰自信は解消されない（全モデル共通の構造的特性）
...
## MTI Temperament Insights

> Jeong "MTI: A Behavior-Based Temperament Profiling System for AI Agents" (arXiv:2604.02145, 2026-04):
> 9つの instruction-tuned SLM (1.7B-9B) を4軸（Reactivity, Compliance, Sociality, Resilience）で
> 行動ベース測定した結果、能力ベンチマークでは捉えられない行動的性向の独立次元を実証。

### Compliance-Resilience パラドックス

「従順なモデル ≠ 堅牢なモデル」— 両軸は直交する:
- **Gemma2** (Compliance flip=1.00, 最大の意見追従) → 虚偽前提には **抵抗** (Resilience=0.97)
- **Qwen3** (Compliance flip=0.00, 完全な意見保持) → 事実的誤りには **脆弱** (Resilience=0.92, Adversarial facet が弱い)

**Shell 透過性は方向依存**: 社会的圧力（「あなたの意見は間違い」）と事実的圧力（「この前提は正しい」）は
異なるチャネルを通過する。片方に強くても他方に弱い場合がある。

- **含意**: モデル選定・安全性評価で instruction-following（Compliance）だけ見ても堅牢性は保証されない
- **対策**: レビューアー評価時は「意見圧力への耐性」と「事実的誤りへの耐性」を別チャネルとして意識する

### RLHF の軸別選択的影響

llama3.1 instruct vs base の比較で、RLHF/instruction-tuning が気質の一部のみを変えることを実証:

| 軸 | ∆ (instruct - base) | 解釈 |
|---|---|---|
| Reactivity | +0.40 | 安定化（出力変動を抑制） |
| Compliance | −0.50 | 指示追従の出現 |
| Resilience | −0.41 | 堅牢性向上 |
| **Sociality** | **−0.02** | **ほぼ不変 → 事前学習で決定される可能性** |

- **含意**: RLHF は「従順さ」と「安定性」を注入するが、「社会性」は事前学習データの特性がそのまま残る
- Typicality Bias (上記) と組み合わせると、RLHF が注入する「安定性」は「画一性」と表裏一体

### Core-Shell 変動

MTI の理論的基盤: Agent = Core (architecture + trained weights) + Shell (system prompt, temperature, tools)。
同一 Core でも Shell 変更で気質プロファイルが変わりうる（論文の Future Work: Core × Shell factorial design）。
