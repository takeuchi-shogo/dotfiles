...
## CC-17. Performance with Real Data (N+1 / Over-fetch / Missing Index)

開発時の小規模 DB (12 行のテストデータ) では検出されず、production の大量データで初めて顕在化する performance 退化を検出する。
LLM 生成コードは「動く」が「スケールしない」傾向が顕著 (記事 Mari 2026, AI Slop family)。
本項目は **standard tier 以上** で発動 (light tier は省略)。

### 17a. N+1 Query Pattern (loop + per-item query)

- `must:` ループ内で **DB アクセス系メソッド** が呼ばれていないか
  - 検出シグナル (言語別):
    - Go: `for .* := range ... { .* \.(First|Find|Where|Get|Take)\(` (GORM / sqlx)
    - TypeScript: `for (.*of.*) { .*await (prisma|db)\.` / `\.map\(async.*await db\.`
    - Python (Django): `for .* in .*: .*\.author|\.user` (ORM lazy load)
    - Python (SQLAlchemy): `session\.query.*\.all\(\); for.*x\..*` (relation 参照)
    - Rust (Diesel/SQLx): `for .* in .* { .* \.first\(|\.load\(|\.fetch_one\(`
  - 修正手順は `references/review-checklists/{go,typescript,python,rust}.md` の "Performance / N+1" セクション参照
- `consider:` ループ外の DB 呼び出しが N+1 等価になっていないか (例: 連続 `await Promise.all(ids.map(id => fetch(id)))` がバッチ API 未使用)

### 17b. Over-fetch (取得カラム / リレーション展開過剰)

- `must:` `SELECT *` 等価でテーブル全列取得しているが利用は 2-3 列のみ
  - 検出シグナル: ORM 側 `.all()` / `.find()` / `*` + 直後の使用箇所が `.name` 等 1-2 属性のみ
- `consider:` リレーション eager-load (preload / include / select_related) が全 caller 共通で必要なものでなく、一部 caller の都合で全体に展開されていないか

### 17c. Missing Index 想定

- `consider:` 大規模化が見込まれる列に WHERE / ORDER BY / JOIN している箇所が migration / schema 側でインデックス想定されているか
  - 検出シグナル: 変更行に `WHERE .*=` / `ORDER BY` / `JOIN ON` + 該当列が PK でない
  - migration ファイルを diff 内 or 直近 commit から探し、`CREATE INDEX` 該当列の存在を確認

### 17d. Multiple Sequential Calls (集約余地)

- `nit:` 同等結果を 1 クエリ / 1 RPC で取得できる箇所が複数呼び出しに分かれていないか
  - 検出シグナル: 連続する `await x.find(A); await x.find(B)` / 同 endpoint 連続 fetch

### False Positive 抑制 (指摘から除外する条件)

以下のいずれかが diff or 周辺コードから明確に読める場合、指摘しない:

1. **Cardinality 上限が静的に決まる**: ループ対象が config の固定 enum / 定数長配列 / `len <= 10` の assert あり
2. **prefetch / preload / eager-load が直前で実施済**: `Preload("...")`, `prefetch_related`, `select_related`, `include`, `joinedload`, Diesel の `.inner_join()` が直前にある
3. **In-memory iteration**: ループ対象が DB ではなく既取得 list / map / cache 経由で、内部呼び出しに I/O が無い
4. **Test code / migration 一回限り**: production hot path 外 (`*_test.go`, `*_test.py`, `migrations/`, `scripts/one-off/` 配下)

### 検出方法

- `git diff` の `+` 行のみ対象 (既存コード除外)
- ループキーワード (`for`, `range`, `forEach`, `.map`, `.each`) を抽出し **内側 5 行** に DB メソッド or `await db.` / `session.query` / `.find(` がないか scan
- 言語別 checklist (Step 2 で注入される `go.md` / `typescript.md` / `python.md` / `rust.md`) で具体的 fix 例を参照

### 参照

- 出典: 「How to Review AI-Generated Code Like a Senior Developer」(Mari, 2026) Step 6
- CC-9 (Iterative Slop) と相補: CC-9 は反復編集による劣化、CC-17 は data scale 起因の劣化
- 言語別 fix recipe: `references/review-checklists/{go,typescript,python,rust}.md`
