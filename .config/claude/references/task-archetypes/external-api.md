...
| 非冪等な書き込みのリトライ | timeout 後にリトライして二重決済・二重登録。**サーバは成功したがレスポンスが届かなかった場合、クライアントからは失敗と区別できない** | FM-002 | 書き込みは idempotency key を送り、サーバ側で重複を排除する。key はリトライ間で同一、リクエストごとに新規 |
| 部分適用の放置 | 複数リソースを更新する呼び出しが途中で失敗し、中途半端な状態が残る | FM-002 | 補償トランザクションを用意するか、単一の冪等な操作に畳む |

## 不変条件

- すべての外部 HTTP 呼び出しにタイムアウトを設定する（デフォルト Client 禁止）
- リトライは指数バックオフ + ジッターで実装する
- **リトライする書き込みは冪等にする**。GET/HEAD など元から冪等な read はそのままリトライしてよいが、POST 等の副作用のある呼び出しは idempotency key を付けるまでリトライ対象にしない（retry と idempotency は必ず対で設計する）
...
| 冪等性テスト | 書き込みを timeout 後に同じ idempotency key で再送し、副作用が 1 回だけ適用されることを確認 | High |
