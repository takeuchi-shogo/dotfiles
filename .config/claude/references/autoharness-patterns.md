...
| Refiner | AutoEvolve の Propose フェーズ | `findings-to-autoevolve.py` は入力整形（findings→L1 Recovery Tips 変換）であり Refiner 自体ではない。改善提案生成は `/improve` の Propose が担う（⚠ findings-to-autoevolve.py は 2026-06-05 退役。decommission-log.md 参照） |
