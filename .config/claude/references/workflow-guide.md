...
**専門レビュー**（変更内容に応じて追加起動）:
- 汎用レビューと **同じメッセージで並列起動** する
- 変更内容を見て該当するものを選択（複数可）:

| 専門レビュアー | トリガー条件 | 観点 |
|---|---|---|
| `comment-analyzer` | docstring/コメントの大幅追加・変更 | コメント腐敗、WHY欠落、不正確な記述 |
| `test-analyzer` | テスト追加・変更、またはテスト不足が疑われる | カバレッジ漏れ、エッジケース欠落、テスト設計 |
...
- 汎用 + 専門の全結果を統合し、重複を除去して最終評価を提示する
- 指摘があれば修正してから次へ
...
| ~50行以下 | `code-reviewer` 1つ + 該当する専門レビュアー |
| ~200行以下 | 汎用2並列 + 該当する専門レビュアー |
| 200行超 | 汎用4並列 + 該当する専門レビュアー全て |

※ 専門レビュアーは変更内容にマッチするもののみ追加（Review セクションのトリガー条件を参照）
...
| コードレビュー（汎用） | `code-reviewer` + `code-reviewer-ma` + `code-reviewer-mu` | 3視点の並列レビュー |
| コメント分析 | `comment-analyzer` | コメント精度・腐敗検出 |
| テスト分析 | `test-analyzer` | テストカバレッジ・設計品質 |
| サイレント障害検出 | `silent-failure-hunter` | エラー握り潰し・危険なフォールバック |
| 型設計分析 | `type-design-analyzer` | 型の不変条件・カプセル化・型安全性 |
...
2. 汎用レビューに加え、変更内容に応じて専門レビュアーを追加起動する（同一メッセージで並列）
3. セキュリティが関わるコード変更は、通常レビューに加えて `security-reviewer` を追加
4. 言語固有の問題は専門エージェント（golang-pro, typescript-pro）に委譲
