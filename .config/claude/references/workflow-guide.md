...
10. **Agent Creation Heuristic** — 同一ドメインのデバッグが context window exhaustion を起こした場合、専門エージェント作成を検討。`skill-creator` スキルでブートストラップし、Symptom-Cause-Fix テーブルをエージェント定義に埋め込んでから再スタートするほうが、unguided session を続けるより速い（Codified Context 論文の知見: エージェントのドメイン知識は 50% 以上が推奨）
