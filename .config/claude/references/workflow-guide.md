...
|--------|---------|------------------|
| **select** | タスクは似ているが難易度やツールが異なる | claude-hooks (Rust, user-prompt) がタスク規模で reviewer 数を選択 |
| **generate** | タスク種類自体が異なり、異なる構造が必要 | `/autonomous` が DAG を生成し worktree 並列実行 |
| **edit** | 実行中に予期せぬ情報が現れ構造変更が必要 | hook がエラー時にルート変更（claude-hooks post-bash 等） |
...
タスクの種別に応じて、最適なエージェントを選択する。Claude モデルの選択 (メイン=Fable / Opus=推論 / Sonnet=実装) は `references/model-routing.md` の Tier 表に従い、組み込み agent (Explore/Plan/general-purpose) には `model` を明示する:
...
3. アーキテクチャ判断は `backend-architect` に委譲
4. 大規模コードベース分析・外部リサーチ・マルチモーダル処理は `gemini-explore` に委譲
5. 通常の `debugger` で困難なエラー分析は cmux Worker (`launch-worker.sh --model codex`) で Codex に深掘り依頼 (`/codex:rescue` は失敗事例あり使わない、詳細は memory `feedback_codex_casual_use.md`)
6. ドキュメントの陳腐化が疑われる場合は `doc-gardener` に委譲
7. UI の状態確認・バグ再現は `ui-observer` に委譲（agent-browser をサブエージェント内に閉じ込め、メインコンテキストを保護）
8. マルチモデル比較や Cloud Agent（非同期長時間タスク）は `/cursor` スキルに委譲
9. **Agent Creation Heuristic (G5)** — デバッグが膠着したら、専門エージェントを作成して再起動する方が速い
...
- **自動推奨**: `claude-hooks` (Rust, `user-prompt`) がキーワードから Async/Scheduled を自動検出し、additionalContext で推奨する
...
- **未一次確認 (記事主張のみ、断定しない)**: reasoning=xhigh / session-scoped 継続 / Auto Mode 併用推奨 — 実挙動は `/effort` 実機 or 公式 docs で要確認。harness にはここまでしか焼き込まない
