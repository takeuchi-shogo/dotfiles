...
S/M/L は規模軸 (相対的工数) と control demand 軸 (説明責任・可逆性) の二軸で判定する。
規模が S でも以下のいずれかに該当するときは M 相当の Gate を強制する (= "control surface" を表に出す):

- 認証・認可・暗号処理の変更
- master ブランチ直変、または不可逆な migration / DROP
- harness ファイル (hook / settings.json / policy script) への変更
- 外部連携契約 (API schema / proto / OpenAPI) の breaking change

判定方針: change-surface-advisor の Critical/High カテゴリは規模に関わらず Plan + Edge Case Analysis + Review Gate を必須化。
理由: control demand が高い surface は relief pressure (自動化欲求) より優先する。
詳細: [change-surface-matrix.md](change-surface-matrix.md), [reversible-decisions.md](reversible-decisions.md)

---

## 2. Wave 1 統合（change-surface-advisor）
