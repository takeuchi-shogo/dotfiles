...
| `error-to-codex.py` | Bash エラー検出 | cmux Worker `launch-worker.sh --model codex`（エラー分析。`/codex:rescue` は失敗事例あり） |
...
| Bash エラー | `error-to-codex.py` → cmux Worker `launch-worker.sh --model codex` で深い分析 |
