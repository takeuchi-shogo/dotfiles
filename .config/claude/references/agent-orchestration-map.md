...
| **Router** | hooks (claude-hooks user-prompt / pre-edit) | キーワード・ファイルパターンから委譲先を自動推奨 |
...
| 条件 | 委譲先 |
|---|---|
| 設計判断・リスク分析・深い推論 | Codex (`codex-plan-reviewer`, `codex-reviewer`, cmux Worker `launch-worker.sh --model codex`) |
...
|---|---|---|
| `claude-hooks` (Rust, `user-prompt`) | タスクキーワード検出 | 各専門エージェント（additionalContext で推奨） |
| `claude-hooks` (Rust, `pre-edit`) | ファイルパターン検出 | 対応する専門エージェント（.md → doc-gardener 等） |
| `claude-hooks` (Rust, `post-bash`) | Bash エラー検出 | cmux Worker `launch-worker.sh --model codex`（エラー分析。`/codex:rescue` は失敗事例あり） |
| `claude-hooks` (Rust, `pre-websearch`) | 大規模分析キーワード | `gemini-explore`（1M コンテキスト活用） |
| `claude-hooks` (Rust, `post-bash`) | テスト失敗 | `test-analyzer`（失敗分析） |
...
| Bash エラー | `claude-hooks` (Rust, `post-bash`) のエラー検出 → cmux Worker `launch-worker.sh --model codex` で深い分析 |
