...
## 無人実行の liveness 対策 (途中停止の回避)

> 出典: sonicgarden "Claude Code で自己改善ループを作った話" (2026-06-05 absorb)。
> Routine がサブスキルを跨いで動くとき、prompt 品質とは別に **「無人実行で途中停止しない」liveness 条件** が要る。以下は無人 triage を 13 日運用して観測されたハマりポイント。daily-health-check のような単一スキル Routine とは別責務 (サブスキル呼び出しを含む Routine で効く)。

1. **判断ポイント直前に「止まらないで」を再掲する**: サブスキルから戻った直後にエージェントが入力待ちに入る。`/loop` や Routine では、各判断ステップの直前にも継続指示を再掲して停止を防ぐ。
2. **サブスキルの最終出力を短い JSON で閉じる**: 出力末尾が長い自然文だとそこで完結扱いされ次ステップに進まない。サブスキルの末尾を `{"status": ..., "suggested_edits": [...]}` のような短い構造化 JSON にして「まだ続く」を構造で示す。
3. **作業ファイルを `.claude/` の外に置く**: 非対話環境で `.claude/` 配下にファイルを書くと許可ダイアログが立ち無人実行が止まる。ステージング用の中間ファイルは `.triage/` 等の外ディレクトリに置き `.gitignore` で管理する。
4. **`allowed-tools` をサブスキル経由まで網羅する**: `TodoWrite` 等の書き漏れがあると、サブスキル経由の呼び出し時に permission dialog が立つ。Routine が直接使うツールだけでなく、呼び出すサブスキルが使うツールも `allowed-tools` に列挙する。
5. **Stop hook の誤発火に状況説明を添える**: 未コミット状態を検査する Stop hook (例: Web の default `stop-hook-git-check.sh`) が `exit 2` を返して停止させることがある。オーケストレータと各サブスキルの **両側** に「これは無人 triage の途中であり未コミットは正常」と状況説明を置く。
...
- [ ] (サブスキルを跨ぐ Routine のみ) 上記「liveness 対策」5 点を確認 — 判断ポイント前の継続指示 / 短 JSON 終端 / 作業ファイルは `.claude/` 外 / `allowed-tools` 網羅 / Stop hook 誤発火対策
