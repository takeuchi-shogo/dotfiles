...
> **2026-06-21 退役注**: `failure-clusterer.py` / `eval-generator.py` / `run_reviewer_eval.py` / `regression-gate.py` は `/improve` 退役 (2026-05-03) に伴い本日削除済。下記図は退役前の構成を歴史記録として残置。Evaluation/Staleness 系の独立スクリプト (`aggregate_benchmark.py` / `eval-staleness.py` / `split_holdout.py`) のみ live。
...
> 全ての改善提案を分析軸ベクトルでタグ付けし、改善方向の偏り・盲点を可視化する。

### 標準 5 軸

| Vector | 意味 |
|--------|------|
| `clarity` | プロンプト・ドキュメントの曖昧さを減らす |
| `brevity` | トークン消費を削減する (情報量を保ったまま) |
| `accuracy` | 出力の正確性・規約遵守を向上させる |
| `coverage` | 検出/対応漏れ (失敗パターン・エッジケース) を減らす |
| `consistency` | 同種タスクで挙動の揺れを減らす |

### Custom Tags (拡張)

標準 5 軸で表現できない場合、`custom:<tag>` 形式で freeform 拡張可能:

- `custom:error-handling` — 特定のエラーハンドリング戦略
- `custom:cost-control` — API コスト管理
- `custom:trajectory-quality` — 中間推論品質
- 他、AutoEvolve サイクル中に発生した固有テーマを記録

設計意図: 5 軸固定で硬直化することを避け、実戦で必要になった軸を Custom で先行運用 → 3 サイクル以上採用された Custom は標準軸への昇格を検討する。

### 必須化ルール

- **全提案 (Phase 2.5 以降の新規生成) に最低 1 つの vector を必須**とする
- Vectors なき提案は Phase 4 進入前に **reject** (autoevolve-core が再生成)
- 既存 entry (`improvement_vectors: null`) は migration 不要、後方互換を維持
- 1 提案あたり最大 3 vector まで (4 つ以上は粒度過多 → 提案を分割)

### 用途

| 用途 | 仕組み |
|------|--------|
| Phase 5 レポートの分析軸別集計 | proposals.jsonl の improvement_vectors を集計し、サイクル間トレンドを表示 |
| Plateau 検出の sub-signal | 同一 vector の提案ばかり続いている場合は探索狭窄のシグナル |
| Commit footer メタデータ (Rule 50) | autoevolve commit footer に improvement_vectors を JSON 化して埋め込む |
...
> **2026-06-21 退役注**: 下記テーブルの「昇格先」列にある `eval-generator → regression gate` / `regression gate` は退役済 (`/improve` 2026-05-03、本日削除)。後継は learned 昇格ループ (`patterns.jsonl` → `/promote-learnings`)。
...
### Signal Routing (action loop 接続)

> 出典: CREAO AI-First 統合 (2026-04-14) — 観測可能な信号は記録装置ではなく action loop に接続されてはじめて神経系として機能する。
> 詳細マップは `references/observability-signals.md` 参照。

record-only な observability 信号を action にルーティングする 3 ルート:

> **実装状態の注意**: 下記 R1/R2/R3 はいずれも **接続予定 (not yet wired)**。`owner` 列は「読み手責任を持つべきコンポーネント」を示す設計意図であり、現時点では action loop への自動接続は未実装。実装タスクは `docs/plans/2026-04-14-creao-absorb-plan.md` の B1 (R3) / B2 (R1) / B3 (R2) を参照。

| ルート | 信号 → アクション | owner (予定) | escalation (action 失敗時) | 状態 |
|---|---|---|---|---|
| **R1** | `failure-clusterer.py` 出力 → `/improve` Phase 1 入力（Gap 2 解消） | `autoevolve-core` | 改善 reject 連続 → pre-mortem-checklist へ | 接続予定 (B2) |
| **R2** | `improve-history.jsonl` の adoption_rate < 0.3 → skill 見直し自動アラート | `autoevolve-core` (Garden phase を実行) | 連続 2 サイクル低下で Governance Level を 1 段階引き上げ（詳細: governance-levels.md） | 接続予定 (B3) |
| **R3** | `friction-events.jsonl` の同一 friction_class 3 連続 → `pre-mortem-checklist.md` トリガ (B1 接続) | pre-implementation hook | 5 連続で Plan 必須化、未対応で session escalate | 接続予定 (B1) |
| **R4** | `dead-weight-{date}.jsonl` の `is_dead_weight=true` → `/improve` Garden フェーズの削除候補リスト | `autoevolve-core` (Garden) | 30日以上 is_dead_weight=true が続けば `[DEPRECATED]` 付与提案、追加 30日で削除提案 | **wired (2026-04-21)** — Rule 51 参照 |

> **owner 表記の統一ルール**: `autoevolve-core` はプロセス名、`/improve` はユーザー起動のスラッシュコマンド。Garden phase は `autoevolve-core` 内のフェーズを指す。表記が揺れる場合、プロセス/スクリプト名を優先する (`observability-signals.md` と合致させる)。

**循環参照防止**: R3 の pre-mortem 起動は 1 セッション 1 回に制限する（pre-mortem 自体が friction-event を生まないように）。

<!-- TODO(R3-impl): session_context.py で "pre-mortem-triggered-in-session" フラグの実装要確認。
     現在は運用ガイダンスのみで実装レベルの強制がない。
     codex-reviewer 指摘 (2026-04-14 review): 実装されるまでは循環参照防止として弱い。 -->

**改善指標** (将来計測):
- stale failure clusters ↓
- friction-events read-to-action rate ↑
...
22. **Self-referential Evaluation Criteria Improvement 禁止** — AutoEvolve が自身の評価基準（`improve-policy.md`, `skill-benchmarks.jsonl`, `benchmark-dimensions.md`, `optimizer-eligibility.md`, `scripts/eval/holdout_accept_gate.py`）を直接変更することを禁止する。評価基準の変更は必ず人間の承認を必要とする。スクリプト/プロンプトの自己改善は Rule 30 で制約付き許可。Bengio 論文: エージェントが自身の報酬関数を変更できる場合、reward tampering が最適戦略になり得る
...
33. **Contrastive Trace Analysis (Glean Trace Learning)** — `/improve` の Step 0 前に `contrastive-trace-analyzer.py` を自動実行する。結果は `situation-strategy-map.md` への追加候補としてユーザーに提示する。候補の自動マージは行わない（⚠ 2026-06-05 退役: contrastive-trace-analyzer.py は no-op 化。/improve 自体も retire 済。decommission-log.md 参照）
...
| **Gather Signal** — トレース検索 | Step 1: Analyze (Critic) | contrastive-trace-analyzer(退役 2026-06-05) + error 集約 |
...
| スクリプト | 入力 | 出力 | 実行タイミング |
|-----------|------|------|--------------|
| `findings-to-autoevolve.py` (R-11, 退役 2026-06-05) | `review-findings.jsonl` | `recovery-tips.jsonl` への L1 昇格 | `/improve` 実行時 |
...
手順は `references/best-of-n-guide.md`（N 並列実行 + スコアリング / Pruning / Escalation）を参照。
...
> /improve v2 で追加。Codex (gpt-5.6-terra) を分析と敵対的レビューの両方で必須使用し、
...
49. **End-to-End Improvement Floor (yamadashy Routines absorb 2026-04-29)** — Phase 4 ADVERSARIAL GATE の ROBUST 判定の必要条件として、以下 3 条件 AND を満たすことを要求する:

    ```
    (1) holdout pass_rate delta ≥ +1pp  (search セットでなく holdout で計測、Rule 47 と連動)
    (2) regression-suite (Rule 5: 既存テストを壊さない) で新規失敗 0 件
    (3) gaming-detector.py で Goodhart フラグなし (Rule 21 / 21_eval_shrinkage / 21_fm_skew / 23 / 29 / 30)
    ```

    1 つでも条件未達なら ROBUST 不可。レポートに未達条件を明示する。

    LLM 品質追跡で deterministic perf benchmark の "2% 直輸入" を採用しない理由:
    - 測定分散が大きく 2% は noise 範囲内のことが多い
    - 評価者ブレ (Evaluator Drift, Rule 48) が毎サイクル変動する
    - 単一指標の最適化はそのまま Goodhart 化しやすい (Rule 21)

    既存の `accept_rate` (Rule 2 / Convergence Detection) は **plateau 検出専用** であり、E2E floor は **ROBUST 判定専用**。両者は別目的で重複しない。E2E floor の閾値 (+1pp) は暫定値で、3 サイクル運用後に分散実測値で再評価する。

    **Holdout 未計測時のフォールバック**: 対象カテゴリに対応する `split_holdout.py` セットが存在しない or 計測不能な場合、条件 (1) を `SKIP` として扱い、条件 (2)(3) のみで ROBUST 判定を行う。SKIP の場合はレポートに `holdout: unmeasured` を必ず明示する (黙示の判定揺れ防止)。SKIP が 3 サイクル連続のカテゴリは、`split_holdout.py` の実装欠落として next-cycle backlog に登録する。

    **初期 3 サイクルのソフトランディング**: Rule 49 導入後の最初の 3 サイクルは **collect モード** (記録のみ、gate しない) で運用する。各サイクルで 3 条件の値を `improve-history.jsonl` に蓄積し、ベースライン分散を観測する。3 サイクル経過後にユーザーが明示的に gate モードへ切り替える。これにより chicken-and-egg (gate なしでデータが集まらない) を回避する。

    **降格条件**: gaming-detector.py の False Positive が 3 サイクル連続で出る場合、E2E floor を **advisory モード** (gate ではなくレポート表示のみ) に降格して別 ADR で再設計する。連続カウントは `improve-history.jsonl` の `regression_summary.gaming_flags > 0 が 3 サイクル連続` で判定する (`tail -3 ... | jq '[.regression_summary.gaming_flags] | all(. > 0)'`)。

50. **成果属性メタデータ必須 (yamadashy Routines absorb 2026-04-29)** — autoevolve commit の footer に以下の JSON メタデータを記載する:
    ...
    - `improvement_vectors`: Improvement Vectors セクションの定義に従う (1-3 個必須、Phase 2.5 以降の新規生成のみ enforce)
    - `holdout_delta.pass_rate`: 符号付き string ("+1.2pp" / "neutral" / "-0.5pp")
    - `regression_summary`: Rule 49 E2E floor の入力と直結

    メタデータなき commit は autoevolve outcome 統計から **fallback parsing** (commit message プレフィックス `🤖 autoevolve(IMP-...)` ベース) で再構築する。認識自体は失敗しない。

    **Migration scope**: 2026-04-29 以降の autoevolve commit のみ enforce する。過去 commit は無条件で fallback 扱いとし、retrofit は行わない。lefthook の commit-msg hook は footer JSON のシンタックス検証を行わない (warn のみ)。

    footer JSON は minified ではなく可読 (200-300 token 上限)。`holdout_delta` / `regression_summary` は LLM 自己申告ではなく、可能な限り `split_holdout.py` / `gaming-detector.py` の出力をそのまま埋め込む (self-reporting integrity リスクは harness-debt-register HD-003 で追跡)。

51. **Build to Delete wiring — dead-weight-scan → Garden 削除候補 (akira_papa_AI absorb 2026-04-21, Codex 最優先 #3)** — `scripts/lifecycle/dead-weight-scan.py` の出力を `/improve` Garden フェーズの removal candidate として必須参照する。手順: (1) `/improve` Garden 開始時に `~/.claude/logs/dead-weight-$(date +%F).jsonl` の存在を確認、(2) 存在しない or 7日以上古ければ `dead-weight-scan.py` を実行、(3) `is_dead_weight=true` のエントリを `removal_candidates` として収集、(4) 30日連続 dead-weight → `[DEPRECATED]` 付与提案、60日連続 → 削除提案を Garden 出力に含める、(5) 判定結果を `~/.claude/agent-memory/metrics/removal-candidates-history.jsonl` に append。根拠: CLAUDE.md の "Build to Delete" 原則が「harness は過渡的技術、削除コストを最小化」と宣言しているが、改善ポリシー本文が not yet wired だった。削除運用が回らないと `lessons-learned.md` 50件上限・CLAUDE.md 150行上限（Rule: Pruning-First）が形骸化する。fail-safe: 人間承認必須、自動削除は絶対禁止、`[DEPRECATED]` 付与は 30日観察後のみ、ユーザーが明示的に "dead-weight 違う" と言った stem は次サイクル以降除外リスト（`~/.claude/agent-memory/dead-weight-exclusions.txt`）で skip

52. **Build to Delete 実行頻度** — dead-weight-scan は週次実行（月曜朝の `auto-morning-briefing.sh` or cron）。`/improve` 実行時に最新 log が 7日以上古ければ on-demand で再スキャン。スキャン結果で flagged なファイルが 20 件を超えた場合、単一 `/improve` サイクルで処理せず Garden フェーズを分割（Rule 1 の 3 ファイル上限遵守）

## 現在のフォーカス

**Pruning-First / 実戦検証型（2026-04-11 更新, pepabo 失敗学習ループ記事由来）**

改善は「追加」より「削る」を優先する。思想面の shift:

- 🟢 **月 1-2 件の本質的改善**: 実戦で再発が観察されたものだけを昇格する
- 🟢 **削除 > 追加**: dead-weight-scan で読まれていないルールを先に削る
- 🟢 **容量上限の強制運用**: `lessons-learned.md` 50 件、CLAUDE.md 150 行、MEMORY.md 200 行
- 🔴 **Anti-Goodhart**: 「改善件数」「提案数」「採用率（最大化目標として）」を KPI にしない — Codified Context と独立に Outcome 指標（design_revision_rate, repeat_investigation_count）を追跡する
  - 例外: 後述の `Convergence Detection` の `採用率 threshold=0.30` は **プラトー検出シグナル**としての使用であり、KPI 最大化とは別目的。プラトー検出後のアクションは「戦略切替」（/improve --deep, /audit, refactor-session）であり、「採用率を上げる」ことではない
- 🔴 **新規追加より既存の削除/昇格/降格**: 既存のルールが dead weight 化していないか、1 サイクルごとに必ず確認

> 履歴: 2026-03-10 「全カテゴリを高頻度で改善する」 → 2026-04-11 現方針に移行
> 理由: Codex 批評 (/absorb pepabo 記事) で「既存は研究知見を吸収する力が強すぎて『実戦で残す』思想を失いつつある」と指摘。「追加より pruning が最優先」への philosophy shift。
> verify: 月次 /improve で `lessons-learned.md` エントリ数が 50 以下に維持されているか確認。超過なら降格/削除を提案する

## Convergence Detection

/improve サイクルの改善幅が収束した場合の自動停止提案（AlphaLab Convergence-Aware Budgeting）。

> 多軸 plateau 検出 (yamadashy Routines absorb 2026-04-29) に伴い、単軸 accept_rate を **3 軸 AND** に拡張。Phase 1 (advisory 警告) と Phase 2 (確定 plateau) の段階運用を採用する。

### 3 軸 plateau 判定

| 軸 | 条件 | データソース |
|---|---|---|
| (1) accept_rate | 直近 5 サイクルの平均 ≤ 0.30 (既存 Rule 2) | `improve-history.jsonl` の `adoption_rate` tail -5 |
| (2) holdout pass_rate | 直近 3 サイクル横ばい ±0.5pp | Rule 47 `split_holdout.py` 出力 |
| (3) MDE 全カテゴリ | 全カテゴリで delta ≤ MDE (Rule 8) | `aggregate_benchmark.py` delta レポート (未実装の場合は SKIP として扱い、(1)(2) のみで判定) |

### 段階運用

| Phase | 条件 | アクション |
|-------|------|-----------|
| **Phase 1 (advisory 警告)** | 3 軸のうち **1 軸でも条件充足** | 警告のみ、改善継続を許可 (Rule 2 の現状動作と同等) |
| **Phase 2 (確定 plateau)** | 3 軸 **全充足** | 戦略切替推奨、下記アクション一覧を提示 |
...
| accept_rate threshold | 0.30 | 軸 (1) の閾値 |
| holdout flatness | ±0.5pp / 3 cycles | 軸 (2) の閾値 |
| min_data_points | 5 | データ不足時はスキップ |
| storage | `~/.claude/agent-memory/metrics/improve-history.jsonl` | 採用率の時系列ログ |

### プラトー時の推奨アクション (Phase 2 確定時)
...
### Trajectory Scoring sub-signal

最終 outcome 軸 (1)(2)(3) に加えて、**proposal の中間品質**も plateau の補強情報として参照する:

- Phase 4 ADVERSARIAL GATE で `VULNERABLE` 比率が連続上昇 → 提案質低下シグナル
- 同一 `improvement_vectors` ばかり続いている → 探索狭窄シグナル

これらは確定条件ではなく Phase 1 警告の補強情報。

### 撤退条件

Phase 2 (3 軸 AND 警告) が実データで 0 件しか出ないサイクルが 1 回以上 → Phase 1 (1 軸 OR 警告) を維持し、Phase 2 を pending として skill 更新を一時凍結する。
