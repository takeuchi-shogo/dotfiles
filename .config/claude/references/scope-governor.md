...
**バグクラスは分類の前に洗う**: accepted finding が単発のミスではなく同じ誤りの繰り返しを示していたら、直す前に現 PR の diff 内に兄弟インスタンスがないか確認する。上表の follow-up が指す「隣接するバグクラス」は**現 PR が触っていないサーフェス**のことで、同じ diff の中に残っている同種の箇所は follow-up ではなく in-scope blocker として扱う。1 件だけ直して残りを次に回すと、その PR はバグクラスが半分残った状態で着地する。

洗う範囲は、既に触っているサーフェス・owner 境界・明確に follow-up 領域と言える箇所で止める。バグクラスの追跡を口実に §3 の相対ゲートを超えてはならない。
