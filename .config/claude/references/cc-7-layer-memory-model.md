...
- **CacheSafeParams**: fork は親のプロンプトキャッシュを共有。CLAUDE.md/settings.json/MCP 接続/tool deny の **セッション途中変更** および **Claude Code 本体 upgrade** はキャッシュを破壊する。**ただし CLAUDE.md の mid-session edit はファイルに保存されるだけで live cache には反映されない** ── 反映は次の `/clear` または restart 時。誤って live cache を壊したくない場合は restart 前に編集を集約する
