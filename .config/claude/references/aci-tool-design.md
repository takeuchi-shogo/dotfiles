...
## AX (Agent Experience) 設計チェックリスト

> ACI はエージェントが使う**ツール**の設計。AX はエージェントが**消費者として利用するサービス**の設計。
> "AX is the new UX" — Zen of AI Coding (Yoav Aviram, 2026)

エージェントが外部サービスを利用する場合、人間向け UI/UX とは異なる設計が必要になる。
API を設計・レビューする際に以下を確認する。

| # | チェック項目 | 理由 |
|---|-------------|------|
| 1 | **構造化レスポンス**: JSON/protobuf 等の機械可読フォーマットを返すか | HTML や自然言語テキストはパース不要なオーバーヘッドを生む |
| 2 | **エラーの機械判定可能性**: エラーコード + 構造化メッセージか | 人間向けの曖昧なエラー文はリトライ判断を困難にする |
| 3 | **冪等性**: 同じリクエストを再送しても安全か | エージェントはリトライを前提に動作する |
| 4 | **権限の最小化**: 必要最小限のスコープで動作できるか | エージェントに広い権限を渡すとリスクが拡大する |
| 5 | **コンテキストサイズ**: レスポンスがトークン予算内に収まるか | 大量データの返却はコンテキストを圧迫する。ページネーション・フィルタを提供する |
| 6 | **発見可能性**: 利用可能な操作をプログラム的に列挙できるか | OpenAPI spec, MCP tool listing 等 |
