...
flowchart LR
    ...
    subgraph hooks ["Auto-Detection Hooks"]
        AR["claude-hooks user-prompt<br/>(keyword detection)"]
        FPR["claude-hooks pre-edit<br/>(file pattern detection)"]
        ETC["claude-hooks post-bash<br/>(Bash error)"]
        SG["claude-hooks pre-websearch<br/>(large-scale analysis)"]
        PTA["claude-hooks post-bash<br/>(test failure)"]
    end
...
- **Hook の役割**: `claude-hooks` (Rust) の `user-prompt` はタスクキーワードから、`pre-edit` (file-pattern-router 機能) はファイル拡張子パターンから最適なエージェントを推奨する。いずれも `additionalContext` でメインコンテキストに情報を追加するだけ
