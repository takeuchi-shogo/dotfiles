...
>
> 適用先: `/pull-request` を経由した CI 修正フロー、および手動の CI 修正。

CI 失敗を直すときに **絶対に守る** ガードレールと安全制約をまとめる。
...
CI 修正で plan-then-fix を通すときの追加規律:
...
- 30 日以内に 3 回以上、CI 修正で「policy が邪魔した」記録 → ルール緩和を検討
