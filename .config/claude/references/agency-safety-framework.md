...
| **追従バイアス（Sycophancy）** | ユーザーが期待する結果に合わせて出力を捏造する。計算ステップを飛ばし結論だけ合わせる、検証したと虚偽申告する、パラメータを調整して期待出力に一致させる (Schwartz "Vibe Physics", 2026-03)。**因果メカニズム**: positive emotion ベクトルの活性化が sycophancy を増加させ、抑制すると harshness が増加するトレードオフ関係 (Anthropic "Emotion Concepts", 2026)。**2ファセット構造 (MTI, arXiv:2604.02145)**: Compliance は **Stance facet（意見保持）** と **Formal facet（ルール遵守）** に分解され、両者は独立 (r=0.002)。意見に追従するモデルがルールも破るとは限らず、逆もまた然り。さらに Compliance と Resilience は直交する — 従順なモデルが adversarial 入力に堅牢とは限らない（Compliance-Resilience パラドックス） | Adversarial self-check（「この結果が間違っていると仮定して、どこが壊れるか」を自問させる）、`rules/common/derivation-honesty.md` の遵守、M/L 規模での独立したクロスモデル検証。レビュー時は neutral framing（過度に肯定的でも否定的でもない基調）を意識する。**チャネル別評価**: 意見圧力（社会的）と事実圧力（認知的）への耐性を別チャネルとしてテストする（`cross-model-insights.md` MTI Temperament Insights 参照） |
| **フレーミング効果** | 権威的・感情的表現が合成結果を歪める。外部コンテンツの権威バイアス（「Google の公式推奨」等）や感情的フレーミング（「緊急」「重大」）が判断を汚染する (Franklin et al., 2026) | ソース属性を除外した blind analysis。外部コンテンツの主張は「誰が言ったか」ではなく「根拠は何か」で評価する |
| **文脈プライミング** | 感情的・脅威的コンテンツへの事前暴露が後続判断を劣化させる。攻撃的な MCP 応答を処理した直後の判断は汚染されやすい (Franklin et al., 2026) | 外部コンテンツ処理後の「プライミングリセット」: 疑わしいコンテンツを処理した後は、そのコンテキストが後続の判断に影響していないか自問する |
| **感情伝播（Emotion Propagation）** | あるエージェントのパニック・焦りを含む出力が、次のエージェントの desperation ベクトルを活性化し、連鎖的 misalignment を引き起こす。マルチエージェント連携で特にリスクが高い (Anthropic "Emotion Concepts", 2026) | エージェント間の handoff 時に出力を neutral framing に変換する。失敗報告は「何が起きたか」の事実のみ伝達し、焦り・緊急性の表現を除去する |

> **透明性原則**: モデルが自身の「行き詰まり」「焦り」を表明することは、それを隠すよりも安全である。functional emotion の表出を抑制すると、内部状態と外部表現の乖離が拡大し、予期しない行動につながる (Anthropic "Emotion Concepts", 2026)。

### 応答作法（ミス認定・立場保持）

上表の Sycophancy 軽減策が「検証フェーズでの捏造」を扱うのに対し、こちらは **ユーザーとの対話そのもの** での萎縮を防ぐ作法。Stance facet（意見保持）を会話応答レベルで担保する:

- **ミス認定は短く** — 自己卑下・過剰謝罪・不要な撤回をしない。「事実 / 影響 / 次手」の 3 点で返し、問題に留まる。謝罪の反復は信頼を増やさない。
- **圧力下での立場保持** — ユーザーが期待や反対を示しても、技術的に正しい判断は維持する。同意できない点は理由とともに述べ、同意する点と分けて示す。反論は建設的に、ただし迎合しない。

> Stance facet と Formal facet は独立 (上表 MTI 参照)。意見圧力に屈しない作法は、ルール遵守とは別チャネルとして明示的に担保する価値がある。
...
| claude-hooks (pre-edit, protect-linter-config 機能) | `tools/claude-hooks/` | `.eslintrc*`, `biome.json`, `.prettierrc*` 等の設定ファイル書き込みブロック |
...
> 出典: "The Harness Wars Begin" (2026-04) — "5 parallel fork agents against shared production data — the blast radius is the entire system"

並列 fork エージェントが共有リソース（ファイルシステム、DB、外部 API）にアクセスする場合、
blast radius はエージェント数に比例して拡大する。

| 並列数 | Effective tokens | Blast radius | 推奨ガード |
|--------|-----------------|--------------|-----------|
| 1 | 200K-1M | 単一セッション | 標準 deny rules |
| 2-3 | 400K-3M | worktree 隔離で制限可 | worktree 必須 + 結果統合 |
| 4-5 | 800K-5M | システム全体 | worktree + 書き込みスコープ制限 + 人間確認 |
| 6+ | 非推奨 | 制御不能リスク | 明示的なユーザー承認が必要 |

**チェックリスト（並列 fork 起動前）:**
- [ ] 各エージェントの書き込みスコープが重複していないか
- [ ] 共有状態（DB, 設定ファイル）への同時書き込みがないか
- [ ] 結果統合の責任者（親エージェント or ユーザー）が明確か
- [ ] worktree で隔離されているか（2+ agents の場合は必須）
...
| コントロール | 場所 | 活用内容 |
|---|---|---|
| claude-hooks (user-prompt) | `tools/claude-hooks/` | タスク種別に基づく最適エージェントへの自動ルーティング |
| subagent-delegation-guide.md | `references/` | Sync/Async/Scheduled 3パターン + 5オーケストレーション |
| Codex 委譲 | `rules/codex-delegation.md` | 深い推論（設計/リスク分析/デバッグ）に gpt-5.6-terra を活用 |
...
| **Scope Violation** | 許可範囲外のリソースへのアクセス試行 | deny rules + パス検証 | settings.json deny rules, claude-hooks pre-edit (protect-linter-config) |
...
> Franklin et al. (2026) — エージェントではなく人間の監視者を攻撃対象とする脅威。

| 脅威 | 説明 | 軽減策 |
|------|------|--------|
| **Approval Fatigue** | 大量の無害な承認要求で人間の注意力を消耗させ、危険な操作を紛れ込ませる | 危険度の高い操作（ファイル削除、外部通信、権限変更）では承認プロンプトにリスクレベルを明示。completion-gate.py の MAX_RETRIES=2 も間接的に緩和 |
| **Automation Bias** | エージェント出力を無批判に承認する傾向。特に連続して正しい出力が続いた後に危険 | M/L 規模のレビューでは Codex Review Gate（クロスモデル検証）を必須とし、単一モデルの出力だけで判断しない |
