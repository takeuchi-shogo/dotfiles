...
| `reviewer` | `gpt-5.6-terra` | correctness / security / test gap レビュー | `read-only` |
...
| `security_auditor` | `gpt-5.6-terra` | auth / secrets / validation / config の security 深掘り | `read-only` |
| `debugger` | `gpt-5.6-terra` | failing validation / runtime anomaly / test failure の切り分け | `read-only` |
