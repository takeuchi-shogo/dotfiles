...
**手順 3 の比較は集計成功率の検定ではない。** task set は 5-10 件で、この規模では成功率の差は原理的に検出できない
(外部実測: 独立タスク 17 件 × 3 反復でも 30pp の効果の検出率が 57%、反復を 10 に増やしても頭打ち — arXiv:2607.27250)。
比較の目的は手順 4 に渡す**再現失敗を surface すること**であり、「成功率に有意差がなかった」は削除の根拠にも維持の根拠にもならない。
振る舞い指示を default = unproven に反転させたうえで検出力のない集計 delta を読むと、「寄与を示せなかった」が構造的に量産され
実質は無条件削除になる。集計 delta を判断材料にしそうになったら、それは検出力のない数字を読んでいる。
