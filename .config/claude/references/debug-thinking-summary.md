...
| `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1` (env var) | adaptive 無効化。**ただし Opus 4.7 では no-op** (公式 docs: "Has no effect on Opus 4.7, which always uses adaptive reasoning")。Opus 4.6 / Sonnet 4.6 のみ有効。本 dotfiles では Opus 4.7 lock-in のため `settings.json` から除外 (2026-05-19) |
