...
2. **NEEDS_FIX**: Important 指摘を修正後、**Fix-Validate ゲート**を通し、**修正差分のみ**を対象に再レビューする（レビューアー構成は修正行数で再スケーリング）
3. **BLOCK**: Critical 指摘を修正後、**Fix-Validate ゲート**を通し、**全変更**を対象にフルレビューを再実行する
