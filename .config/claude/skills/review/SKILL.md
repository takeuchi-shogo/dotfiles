...
- **worktree 内では symlink 越しに main repo 旧版を読む**: `~/.claude` 経由のパスはレビュアーから main repo の旧 snapshot を指してしまい false positive BLOCK が出る (memory: `feedback_worktree_review_symlink_stale.md`)。worktree レビューでは絶対パスを明記し、BLOCK は実ファイルを grep で反証してから採否を決める
