...
- ドメイン判定: 変更ファイルパスの第1階層 → 対応する `.claude/data/review-learnings/<domain>.md`
  - `product/recording/**` → `recording.md` / `proto/**`・`protosdk/**` → `proto.md` / `frontend/**` → `frontend.md` / `admin/**` → `admin.md` / その他 Go backend → `backend.md`
- cross-cutting チェックリストと同様に、該当ドメインの md を追加注入する（複数ドメインに跨る変更は該当する全 md を注入）
- 全体像と出典内訳は `.claude/data/review-learnings/index.md` を参照
...
| 依存・設定変更     | `security-reviewer`     | `package.json`, `go.mod`, `Cargo.toml`, `pyproject.toml`, `*.lock`, `.yaml`, `.toml` のファイル変更（triage-router.md の dependency-config 責務ドメインと同期） |
