...
WebSearch/WebFetch が使えない環境でも、このスキルの精神を守る:

1. **内蔵知識で代替評価**: よく知られたライブラリを列挙し、特徴・トレードオフを比較
2. **採用シグナルの代替指標**: GitHub Stars、npm weekly downloads の概算値を知識ベースで提示
3. **必ず比較表を出す**: 最低2つの選択肢を比較してから推奨する
4. **「調査を推奨」で逃げない**: ツールが使えなくても、既知の選択肢から推奨を出す

**重要**: 「検索できないので実装に進みます」は NG。検索ツールがなくても、知識ベースの比較評価は必ず行う。
...
- **質問で逃げる**: 「どのプロジェクトですか？」と聞く前に、まず選択肢の調査・比較を行う
- **空の回答**: ツール制約を理由に何も出力しない（Fallback セクション参照）
