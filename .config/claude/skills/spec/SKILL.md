---
name: spec
description: "Prompt-as-PRD を生成する。アイデアを構造化プロンプトとして記述し、docs/specs/ にバージョン管理する。agent にそのまま渡せる実行可能な仕様書を作成。Triggers: '仕様書', 'spec', 'PRD', '要件定義', '仕様を書いて', 'what to build'. Do NOT use for: 実装込みのフルフロー（use /epd）、曖昧な Issue の明確化（use mattpocock-skills:grilling）、プロトタイプ検証（use /spike）。"
origin: self
...
- リスク・不変条件（壊してはいけない既存挙動）
- 検証方法（テスト・ベンチマーク・スクリーンショット等）
...
| 6 | **エッジケース** | 「空入力・最大値・並行実行・部分失敗・再実行時はどうあるべきか？」 |
| 7 | **セキュリティ** | 「認証は必要？ A) 不要 B) API キー C) OAuth」 |
| 8 | **パフォーマンス** | 「性能が悪化すると困る経路は？許容回帰は何%？ A) <100ms B) <1s C) 予算なし」 |
| 9 | **互換性・既存統合** | 「既存 API/CLI/設定/保存データとの互換性は必要？ Breaking change のリスクは？」 |
| 10 | **スコープ境界** | 「V1 で必須なのはどこまで？ V2 に回せるものは？」 |
| 11 | **リスク・不変条件** | 「壊してはいけない既存挙動を3つ挙げると？」「絶対に守る invariant は？」 |
| 12 | **影響範囲** | 「diff 外に波及するファイル・設定・ドキュメントは？」「変更すべきだが忘れがちなものは？」 |
| 13 | **検証証拠** | 「この変更が正しいと示す最小の証拠は？」「テスト・スクリーンショット・ベンチマーク？」 |
...
- **Top 5 品質質問**（M/L 規模で必ず1つ以上含める）:
  1. 「壊してはいけない既存挙動を3つ挙げると？」
  2. 「正常系以外で守りたい失敗時の挙動は？」
  3. 「互換性を保つべき公開面はどこ？」
  4. 「性能が悪化すると困る経路は？許容回帰は何%？」
  5. 「この変更が正しいと示す最小の証拠は？」
...
spec ファイルを保存する前に、以下 5 項目を verify する。1 項目でも No なら修正してから save する。

1. **意図網羅**: ユーザーが要求した requirements と acceptance criteria を **過不足なく** 反映したか？ (スコープクリープ・取りこぼし双方を検査)
2. **形式**: frontmatter (status/owner/priority) + 必須セクション (Problem / Requirements / Acceptance Criteria / Non-Goals / Open Questions) が揃っているか？
3. **具体性**: 条件・閾値・依存先・データ型が具体的か？ `precision-ceiling.md` の上限 (DB スキーマ・擬似コード) を超えていないか？
4. **decision-oriented tone**: narrative ではなく decision-oriented になっているか？ 「〜について検討する」ではなく「〜を採用する／〜は Non-Goal とする」と書けているか？
5. **実装可能性**: 実装担当が **追加質問なし** でこの spec だけから実装に着手できるか？ 曖昧な箇所が残っていれば Open Questions に明記したか？
