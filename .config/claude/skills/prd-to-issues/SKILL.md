...
5. **各 Issue に Execution Mode を付与** — HITL (人間判断必要) / AFK (エージェント自走可能) を明示し、AFK 比率を最大化する
...
## Execution Mode
- **HITL** (Human-In-The-Loop): 人間判断が必要 — 仕様未確定 / 設計選択 / プロダクト判断が含まれる
- **AFK** (Away-From-Keyboard): エージェント自走可能 — インターフェース確定済み、テストで pass/fail を判定できる
...
1. [HITL][リスク高] {タイトル} — {概要}（blocking なし → 最初に着手可能 / 仕様確定が必要）
2. [AFK] {タイトル} — {概要}（blocked by #1）
3. [AFK] {タイトル} — {概要}（blocking なし → #1 と並列可能）
...
