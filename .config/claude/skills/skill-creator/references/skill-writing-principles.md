...
- SKILL.md の first screen（冒頭 50 行以内）は trigger / usage / next-read pointers に絞る — 詳細は reference に逃がす
...
- `category`: スキルの分類。以下から選択:
  ...
  - `meta` — skill-creator, skill-audit, improve
...
## 10. 命名規約 — 動詞+名詞を基本形とする

スキル名・コマンド名は **動詞+名詞** パターンを基本形とする。出典: akira_papa_AI 「Obsidian × Claude Code 設計パターン」(2026-03) で経験則として提示。当 dotfiles の既存スキル（`fix-issue`, `create-issue`, `review-pr` 等）も無意識にこの形に寄っており、明文化する。

**Good（動詞+名詞）:**
- `fix-issue`, `create-issue`, `review-pr`
- `weekly-review`, `daily-report`, `check-health`
- `analyze-tacit-knowledge`, `compile-wiki`

**Bad（名詞のみ / 動詞のみ / 曖昧）:**
- `issue`（名詞のみ — 何をするか不明）
- `fix`（動詞のみ — 何を対象か不明）
- `smart-helper`（曖昧 — 役割が読めない）

**例外:**
- 既存の定着したエコシステム名（`codex`, `gemini`, `claude-api`）は変更しない
- pipeline 型スキル（`epd`, `spike`, `rpi`）は略語として許容

**判断フロー:**
1. スキル名を命名する前に、`{動詞}-{名詞}` の形で書けるか試す
2. 書けない場合、スキルの責務が不明瞭な可能性 → 責務を絞り直す
3. それでも動詞+名詞で書けないなら理由を frontmatter の `naming_rationale` に記録

## 11. トップレベルコマンド 5-9 個制約 — 認知負荷上限

スキル作成時、**動詞（プレフィックス）のトップレベルは 5-9 個に抑える**。

出典:
- Miller 7±2（短期記憶容量）
- Hick の法則（選択肢増加で判断時間が対数的に増大）
- Gemini 補完リサーチ (2026-04)：フラットな 35 コマンドは認知過負荷、Git のような 3-5 動詞の階層化が成功事例

**Good（動詞セットを絞る）:**
- Git: `commit`, `push`, `pull`, `branch`, `merge`（5 動詞 × 多数のオプション）
- 当 dotfiles: `create-*`, `fix-*`, `review-*`, `analyze-*`（プレフィックスが動詞）

**Bad（フラット爆発）:**
- 35 個のトップレベルコマンドがフラットに並ぶ → 発見性低下、選択麻痺
- 類似スキル（`daily-note-create`, `daily-note-update`, `daily-note-archive`）が独立コマンドとして乱立

**判断フロー:**
1. スキルを追加する前に、既存のどの動詞プレフィックスに属するか確認
2. 既存動詞に収まらない場合、動詞プレフィックスの総数を数える
3. 5-9 個を超えそうなら、類似スキルに吸収 or 階層化（`verb:subcommand` パターン）

**当 dotfiles の運用指針:**
- 現在のトップレベルコマンドは `/improve`, `/rpi`, `/epd`, `/spike`, `/review`, `/commit` 等多数 → 動詞プレフィックスで再集約する候補を定期的に洗い出す（`/improve` で検知）
- 新規スキル追加時は「既存の weekly-review に Phase 追加」「既存の review skill の mode 追加」を先に検討

## 12. 規範的フレーミング優位 — 禁止より推奨

指示文は **「〜すべき」「〜する」という規範的フレーミング** を基本とし、禁止形（「〜するな」「〜禁止」）は最小化する。

出典:
- Anthropic Constitutional AI (2026 ガイドライン): 規範的フレーミング > 禁止的
- Mirror Effect r=0.925（AI はユーザーの記述スタイルを反映する傾向）
- akira_papa_AI (2026-03): 「禁止より推奨」を 5 設計原則の 1 つとして明示

**Good（規範的）:**
- "テストを先に書く"
- "search-first workflow を使う"
- "Write directives, not wisdom"

**Bad（禁止的、機械的に変換する）:**
- "テストなしでコードを書くな" → "テストを先に書く"
- "勝手に refactor しない" → "ユーザーの要求に一致する変更のみ行う"
- "推測するな" → "不明点は明示的に質問する"

**変換ルール:**
1. 指示文を書く前に `禁止形 → 規範形` に変換できるか試す
2. 変換後、行動の向きが明確になっていれば採用
3. 例外: Anti-Patterns セクション（§ 6 Anti-Patterns セクションの標準化）は ❌/✅ テーブル形式で禁止形も許容（違反例の警告目的）

**例外領域（禁止形を保持すべき場面）:**
- 安全 policy（`careful` skill, deny rules）— "rm -rf を実行しない" は明確な禁止
- Anti-Pattern テーブル（対比のため）— "❌ Don't" 列は禁止形で記載
- Hook の block メッセージ — 違反を止める目的で禁止形が適切

**CLAUDE.md / rules/ の監査方法:**
- grep で `禁止`, `するな`, `しない` を検出
- 各ヒットについて規範形に変換可能かを評価（変換できないのは上記例外のみ）
- `/improve` で定期的にスキャン候補
