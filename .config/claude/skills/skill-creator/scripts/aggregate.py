...
def collect_eval_data(
    ...
    for config_name in ("with_skill", "without_skill"):
        ...
        if config_grading is None:
            warn(
                f"Skipping {eval_dir.name}/{config_name}: "
                "missing in grading.json"
            )
            return None
...
def aggregate_configuration(
    ...
    return {
        ...
        "mean_duration_ms": round(statistics.mean(durations))
        if durations
        else 0,
        "evals": evals,
    }
...
def generate_markdown(benchmark: dict[str, Any]) -> str:
    ...
    lines.append(
        f"- **Recommendation**: {benchmark['recommendation'].upper()}"
    )
    ...
    for config in benchmark["configurations"]:
        stddev_str = f" ({config['pass_rate_stddev']:.2%} sd)" if config["pass_rate_stddev"] > 0 else ""
        lines.append(
            f"| {config['name']} "
            f"| {config['eval_count']} "
...
def main() -> None:
    ...
    print()
    print("=== Result ===")
    print(f"  With Skill:    quality={with_config['mean_quality_score']:.1f}, "
          f"pass_rate={with_config['pass_rate']:.1%}")
    print(f"  Without Skill: quality={without_config['mean_quality_score']:.1f}, "
