...
1. Phase 1 抽出結果の **手法リスト** を抽出 (Phase 1 出力 JSON の `手法` フィールド)
...
3. novel_methods = current_methods - union(prior_methods)
   - 完全一致だけでなく **意味的同等** で判定 (例: 「IPARAG 採用」と「9-folder 構造採用」は同一)
   - 過去 absorb で **明示的に Reject 済み** の手法 (例: one-folder vs IPARAG) は **再評価対象** として novel に含める (新しい角度や追加根拠の可能性)
4. `delta = |novel_methods|` を記録
...
**SATURATED-but-novel / SATURATED-borderline (delta >= 1):**
...
3. **Phase 2.5 (Codex+Gemini) は省略可** — light flag。ただしユーザーが希望すれば実行 (要 AskUserQuestion で確認)
...
log.md 追記時の operation は `ingest (light Phase 2)` とする (`ingest-skip` ではない)。
