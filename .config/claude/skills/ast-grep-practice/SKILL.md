...
11 ready-to-copy YAML snippets are catalogued in
[`references/practical-rule-examples.md`](references/practical-rule-examples.md).

| 言語 | 例 |
|---|---|
| TypeScript | `as any` cast 禁止 (detection-only) / 旧 API → 新 API rewrite / `lodash` 全 import 禁止 / React component 内 `fetch` 直接呼び禁止 |
| Rust | `unwrap()` 禁止 (test 外) / `unsafe` block flag / `println!` → `log::info!` (lib のみ) |
| Go | `_, _ := f()` で error 無視検出 / `os.Open` 後の `defer Close()` 漏れ検出 |
| Python | bare `except` 禁止 / `print` → `logger.info` (src 配下) |
...
- [references/practical-rule-examples.md](references/practical-rule-examples.md) — 11 ready-to-copy YAML snippets (TypeScript / Rust / Go / Python)
