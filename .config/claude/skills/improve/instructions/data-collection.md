# Data Collection (Phase 1: COLLECT)

> Phase 1 は 3 ステップに統合。旧 Step -0.5〜3 の 7 サブステップを簡素化。

## Step 1: データ可用性チェック + Git Context
...
> AutoAgent (2026-04): スコアのみではトレースありと比較して改善率が大幅に低下。
> 「なぜ改善したか」の理解が「改善した事実」の知識と同等に重要。

学習データを分析する際、以下を常に紐付ける:
- **スコア**（成功/失敗、ターン数、エラー回数）と **トレース**（具体的な推論過程）
- 改善提案の根拠は「スコアが悪い」ではなく「トレースのここで判断を誤った」で示す
- `session-trace-store.py` の生トレースを keyword grep で参照し、要約に頼らない
