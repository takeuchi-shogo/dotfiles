...
3. **Proposer**: `autoevolve-core (phase: improve)` を起動。H（`proposer-context --skill {target}`）+ 失敗トレースを注入。AP-1〜4 チェック
   - `--single-change` モード（デフォルト）: **1つの変更のみ提案**。変更を仮説として明記（例: "見出しに数値を強制するルールを追加"）
...
- `checklist_pass_rate` >= 95% が 3 連続 → 目標スコア達成で終了
...
| Iter | 対象スキル | 提案 | Gate 判定 | A/B delta |
|---|---|---|---|---|
| 1 | {skill} | {hypothesis} | {verdict} | {delta} |
...
`--evolve` ループは各ラウンドの試行を `changelog.md` に自動記録する。
将来のモデルにこの changelog を渡すことで、過去の試行を踏まえた改善が可能になる。

```markdown
# Skill Evolution Changelog: {skill-name}

## Round 1 (YYYY-MM-DD)
- **Hypothesis**: {何を変えようとしたか}
- **Change**: {SKILL.md のどこをどう変えたか}
- **Result**: KEEP / REVERT (checklist_pass_rate: X% → Y%, delta: ±Npp)
