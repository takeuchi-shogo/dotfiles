...
**Agent ツールで `autolearn` エージェントを 4 並列起動** し、以下のカテゴリを同時に分析する:
...
**Agent ツールで `knowledge-gardener` エージェントを起動** し、以下を実行させる:
...
**Agent ツールで `autoevolve` エージェントを起動** する。
...
- 1 サイクルで最大 3 ファイルの変更制約を守る（autoevolve エージェントの制約）
