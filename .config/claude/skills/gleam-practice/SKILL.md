...
`gleam_otp` は Erlang `gen_server` の statically-typed wrapper (`:gen_server.call/cast` 直呼び禁止)。
pure state + actor wrapper の 2 層構造、3 種の supervisor (`static` / `factory` / `dynamic`)、`OneForOne` + named actor。

完全な actor wrapper / supervision tree 例 + guidelines: [`references/otp-pattern.md`](references/otp-pattern.md)

## JSON Codec (`gleam/dynamic/decode` + `gleam/json`)

`gleam_json` v2+ の現在の慣用形 (decode / encode + LSP Code Action `gleam-language-server` v1.2+ でコード生成可)。

完全な decode / encode サンプル: [`references/json-codec.md`](references/json-codec.md)
