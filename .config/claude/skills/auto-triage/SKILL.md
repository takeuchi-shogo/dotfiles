...
## Wave3 再開時の再注意 failure modes (academic 裏付け)

arXiv:2606.20363 (Hao & Li 2026-06-18, GUI CUA で skill auto-mining を試みた diagnostic study) は、trajectory→skill mining が **readable structure を出しても cross-domain policy transfer に至らない** ことを self-disclose (GRPO 18.5%→20.5%, trivial frequency prior 以下のメトリクスあり)。失敗主因として論文が特定した 3 つは **Wave3 を再開する場合に必ず再検査すべき**:

1. **Boundary detector insufficient** — 軌跡の意味境界検出が雑だと segment が無意味化する。dotfiles は CFS 検出のみで深追いしない方針 → 強化方向に進まない判断を維持
2. **Orderless segment representation insufficient** — segment を「集合」として扱うと順序情報が失われ、行動シーケンスの意味が崩れる。failure-clusterer (FM-code grouping) を sequence 情報込みに拡張する誘惑があっても **見送る** (transfer 失敗の主因と論文が特定)
3. **Offline reward model insufficient** — 過去ログから学んだ報酬モデルは新ドメインで信頼できない。Wave3 の判定根拠を過去 ledger だけに依存させない (calibration を都度実行する設計を維持)

詳細: `docs/research/2026-06-20-skillmd-trajectory-mining-absorb-analysis.md`
