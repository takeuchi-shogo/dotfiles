...
Opus の仮定リストを cmux Worker 経由で Codex に渡して批評してもらう (`Skill(codex:rescue)` および `Agent(codex:codex-rescue)` は失敗事例あり使わない、詳細は memory `feedback_codex_casual_use.md`):

```bash
~/dotfiles/scripts/runtime/launch-worker.sh --model codex --task "<仮定リスト + 以下の批評観点>"
```
