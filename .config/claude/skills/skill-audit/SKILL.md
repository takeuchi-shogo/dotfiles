...
1. `~/.claude/agent-memory/learnings/skill-executions.jsonl` を読み、過去 30 日間の各スキルの実行回数を集計する (path は `learnings/` サブディレクトリ込み — 親ディレクトリ直下に同名ファイルは無いので注意)
...
3. `~/.claude/agent-memory/learnings/skill-executions.jsonl` が存在しない、またはデータ不足の場合はこのステップをスキップし、5D スキャンのみで判定する
...
- **統計的検出力不足**: 反復回数を増やしても検出力はほとんど買えない。独立タスク数と反復回数を分けて報告し、増やすなら反復ではなくタスクを増やす。外部実測では独立タスク 17 件 × 3 反復で 30pp の効果ですら検出率 57%、反復を 3→10 に増やしても 15pp 効果の検出力は 13%→58% で頭打ちだった (arXiv:2607.27250)。必要件数はタスクの分散で変わるので、この論文の数値をそのまま自分の閾値にしない
- **床/天井への張り付き**: baseline の pass rate が 0 か 1 に張り付いたタスクでは、どんな介入も差を出せない。A/B の前に baseline の pass rate 分布を見て、0 < pass < 1 のタスクが含まれているかを確認する。ここでいう borderline は Step 2 の borderline プロンプト (ドメイン境界の類型) とは別物で、**その skill の pass rate が中間帯にあるタスク**を指す。同じタスクでもモデルが変われば帯から外れるため、モデル更新後は分布を取り直す
