...
## 上位ルール: codex:rescue は使わない

**`Skill(codex:rescue)` および `Agent(codex:codex-rescue)` は両方失敗事例あり、使用禁止**。Permission Storm (15-30 prompts/call) + Silent Stall (Considering citation strategy reasoning loop) + 6-hop chain (観察不能) + orphan job 蓄積 (retention 機構なし) のため。

正規パス (優先順):

1. **対話ラリー / セカンドオピニオン / ブラッシュアップ系**: cmux Worker — `scripts/runtime/launch-worker.sh --model codex --task "..."`
2. **単発投げ・読み取り専用**: `codex exec --skip-git-repo-check -m gpt-5.5 --sandbox read-only --config model_reasoning_effort="xhigh" "..." 2>/dev/null`
...
4. **セキュリティ深掘り**: `codex exec --skip-git-repo-check -m gpt-5.5 -p security "..."`
...
> ```
> docs/policy script の推奨メッセージが launch-worker.sh のみを示している場合は読み手が判断する責任。
> **codex-reviewer subagent と codex:codex-rescue subagent の差**: codex-reviewer は codex CLI を内部で短時間 spawn する確定的 wrapper (input/output ともに structured)、codex:codex-rescue は `--background` で codex を長時間 detach し reasoning loop で stall するインタラクティブ経路。両者は「subagent → codex」という共通点はあるが、ライフサイクル管理と observability が決定的に異なる。前者は安全、後者は廃止。
> **launch-worker.sh の sandbox**: codex case は default sandbox (read-only ではない)。read-only 保証が必要なら正規パス 2 を使う。

詳細は memory `feedback_codex_casual_use.md` を参照。
