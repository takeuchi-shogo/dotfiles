...
- lint / 型エラーを inline の `eslint-disable` / `@ts-ignore` / `@ts-expect-error` で黙らせない — 根本を直す。エージェントはエラーを消す最短経路を選ぶので、禁止しないと型が守る範囲が静かに減る
- エージェントが inline で書いてよい唯一の例外は、型テストで型エラーの発生自体を検証する `@ts-expect-error`。設定ファイルでは代替できないので inline のまま残す
- それ以外で恒久的に残す例外は **lint 設定ファイル側に 1 エントリ 1 理由で置く**。理由が消えたらエントリも消す（inline コメントは半年後に「これはまだ必要か」を判断できない）。ただし設定ファイルは `protect-linter-config` の保護対象で **エージェントは編集できない** — 追加は人間が承認し、対象ルール 1 件・対象パス最小に限って手作業で入れる
- この経路でも、エラーを消すために閾値やルールを緩めるのは禁止（ADR-0004「Fix code, not rules」）。設定に置けるのは「意図的な例外」であって、規約の緩和ではない
