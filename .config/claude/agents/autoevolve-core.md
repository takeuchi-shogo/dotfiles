...
```jsonc
{
  ...
  "parent_id": "IMP-YYYY-MM-DD-NNN | null",        // 親提案の ID（改良元がある場合）
  "novelty_score": "0.0-1.0 | null",               // 既存提案との最大類似度の逆数
  "similar_proposal_ids": ["IMP-..."],              // 類似度 > 0.5 の提案 ID リスト
  "mutation_type": "refine | pivot | novel | null", // 探索タイプ
...
1. コミットメッセージから `IMP-YYYY-MM-DD-NNN` を抽出して proposal_id を特定
2. proposals.jsonl の該当エントリの outcome を更新:
   - merge → `merged`
   - revert → `reverted`
   - decline → `declined`
3. outcome_delta を計測:
...
5. rejected-patterns.jsonl との統合:
   ...
   - analysis_snippet を rejected-patterns の reason フィールドにも反映
