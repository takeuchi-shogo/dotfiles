...
pprof ガイド: `references/go-pprof-guide.md`
モダナイゼーション: `references/go-modernize-checklist.md`
プロジェクト構成: `references/go-project-layout.md`
CI パイプライン: `references/go-ci-pipeline.md`
...
### TROUBLESHOOT モード
- バグ、クラッシュ、デッドロック、予期しない動作の調査時に有効化
- **ultrathink で深い推論を行う** — 急いだ推論は症状修正に留まる
- Golden Rules に従い、根本原因を特定してから修正する
- 出力: 根本原因の特定 + 修正 + 回帰テスト
...
1. **エラーメッセージを最初に読む** — Go のエラーはファイル:行番号付き。全文読んでから動く
2. **修正前に再現する** — 失敗するテストを書き、最小再現例を作り、`git bisect` で破壊コミットを特定
3. **計測せずに推測するな** — pprof > 直感、race detector > 推論、ベンチマーク > 仮定
4. **1仮説ずつ** — 3つ同時に変えると何も学べない。1変更→計測→確認
5. **根本原因を見つける** — バンドエイド修正は不可。「なぜ」を5回繰り返す
6. **diff だけでなくコードベースを調査** — 呼び出し元、上流バリデーション、ミドルウェアを確認

### Quick Decision Tree

| 症状 | 最初の一手 |
|------|-----------|
| コンパイルエラー | `go build ./... 2>&1`, `go vet ./...` |
| 不正な出力 / ロジックバグ | 失敗テスト → エラー処理・nil・off-by-one 確認 |
| ランダムクラッシュ / panic | `GOTRACEBACK=all ./app`, `go test -race` |
| 時々失敗 | `go test -race ./...`, `-count=10` でフレーク検出 |
| ハング / フリーズ | goroutine dump: `curl localhost:6060/debug/pprof/goroutine?debug=2` |
| CPU 高負荷 | pprof CPU profiling → `references/go-pprof-guide.md` |
| メモリ増大 | pprof heap profiling → `references/go-pprof-guide.md` |
| 高レイテンシ / p99 スパイク | CPU + mutex + block profiles |

### Red Flags（デバッグの危険信号）

以下に気づいたら STOP して Step 1 に戻る:
- 「とりあえずの修正」— "later" は来ない。根本原因を探す
- 複数同時変更 — 1仮説ずつ
- 原因不明で修正を提案 — 「nil チェック入れたら直るかも」は推測
- 修正するたびに新問題 — 症状治療中。本当のバグは別の場所
- 3回以上の修正試行 — メンタルモデルが間違い。最初からデータフローを追い直す
