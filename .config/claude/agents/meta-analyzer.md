...
11. **HITL パターン分類** (LayerX 非対称評価分析):
   `metrics/session-metrics.jsonl` および `learnings/friction-events.jsonl` から
   HITL（ユーザー介入）イベントを抽出し、セッション横断でパターンを分類:
   - **Gini 係数（集中度）**: HITL がセッション間で均一(Gini<0.3)か一部に集中(Gini>0.6)か
   - **変動係数 CV**: セッション内 HITL 頻度のばらつき。CV<1 は分散型、CV>1.5 は高変動
   - **Burstiness B**: HITL の連続集中度。B>0.3 かつ CV>1 でランダムバースト型
   - **FLI（フロントローディング指数）**: セッション前半の HITL 比率。FLI>0.7 で前半集中型
   パターン別の改善示唆:
   - **集中型** (Gini>0.6): 特定タスクタイプでの自動化候補を特定
   - **分散型** (CV<1): ベースラインの HITL 率として受容。改善は全体最適で
   - **フロントローディング型** (FLI>0.7): Plan 段階の品質向上で HITL を先送りせず解決
   - **ランダムバースト型** (B>0.3, CV>1): 判断疲労リスク。セッション分割を推奨
   データソース: `friction-events.jsonl`（type フィールドで HITL イベントを抽出）
   データ不足時（セッション < 10）: `INSUFFICIENT_DATA` と報告
...
## HITL パターン分析
- パターン分類: {集中型/分散型/フロントローディング型/ランダムバースト型}
