...
### 7. ハーネス破壊的変更（.config/claude/ 配下の変更時のみ）

この変更が既存のハーネスインフラを壊さないか。

- hook の matcher regex が変更後のツール名/ファイルパスに一致するか
- settings.json が参照するスクリプトパス・エージェント名が実在するか
- symlink のターゲットが移動・リネームで切れないか
- 他の skill/agent/reference が変更対象ファイルを参照していないか
