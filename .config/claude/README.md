...
- **アーキテクチャ俯瞰図 (HTML)**: [docs/architecture.html](docs/architecture.html) — Hooks / Skills / Agents の関係性・カテゴリ・役割・使いどころをビジュアルにまとめた地図 (ブラウザで開く)
...
graph TB
    ...
    subgraph execution["Execution Layer"]
        direction TB
        subgraph hooks_box["Hooks (9 events / 100+ scripts)"]
            direction LR
            H_RT["Runtime (42)<br><small>format, session, checkpoint</small>"]
            H_PO["Policy (40)<br><small>golden-check, error-to-codex</small>"]
            H_LC["Lifecycle (7)<br><small>plan-lifecycle, doc-garden</small>"]
            H_LR["Learner (17)<br><small>session-learner</small>"]
        end

        subgraph skills_box["Skills (121)"]
            direction LR
        ...
        PLUGINS["Plugins (16)<br><small>superpowers, pr-review-toolkit, etc.</small>"]
    end

    subgraph external["External CLI Tools"]
        direction LR
        CODEX["Codex CLI<br><small>gpt-5.5 / 深い推論</small>"]
...
| **Deep Reasoning** | Codex CLI (gpt-5.5) | 400K | 設計・推論・複雑なデバッグ | `rules/codex-delegation.md` |
...
├── references/                   # 参照ドキュメント (162個)
│   ├── review-checklists/        # 言語別レビュー基準
│   └── blueprints/               # ワークフローブループリント
├── scripts/                      # Hook スクリプト
│   ├── runtime/     (42個)       # セッション管理・フォーマット
│   ├── policy/      (40個)       # ポリシー強制・エラー検出
│   ├── lifecycle/   (7個)        # 計画追跡・ドキュメント管理
│   ├── learner/     (17個)       # 学習データ収集・分析
│   └── lib/         (14個)       # 共有ユーティリティ
├── templates/                    # テンプレート
└── docs/
    ├── architecture.html         # アーキテクチャ俯瞰図 (HTML)
...
**9 イベント**にバインドされ、スクリプトは `scripts/` 配下に policy 40 / runtime 42 / lifecycle 7 / learner 17 / lib 14 本。

> 以下の対応表は代表例。**配線の正は `settings.json` が source of truth**、俯瞰は [docs/architecture.html](docs/architecture.html) を参照。
...
| **PostToolUse** (Bash) | `claude-hooks` (Rust, `post-bash`) | 出力退避、エラー分析、テスト解析、計画追跡、停滞検知（統合） |
...
## Skills カテゴリマップ (121個: native ~57 + installed 64)
...
## Agents カテゴリマップ (23個 + _archived 10)
...
| `typescript-reviewer` | TS/JS 専門 (rules+checklist 両読み・Result/層強制・React) |
...
| プラグイン | 提供元 | 機能 |
|-----------|--------|------|
| `superpowers` | claude-plugins-official | worktree, 並列エージェント, TDD 等のワークフロー |
...
| `ralph-loop` | claude-plugins-official | Ralph Loop（反復自律実行） |
| `codex` | openai-codex | Codex 連携 plugin |
| `obsidian` | obsidian-skills | Obsidian 連携 skill 群 |
| `mattpocock-skills` | mattpocock-skills | TDD/triage/prototype 等の skill 群 |
| `terraform-skill` | antonbabenko-terraform | Terraform 支援 |
| `warp` | claude-code-warp | Warp 連携 |
| `datadog` | kw-marketplace | Datadog 連携 |
| `gopls-lsp` / `rust-analyzer-lsp` / `typescript-lsp` | claude-plugins-official | 言語 LSP サポート |
...
| サーバー | 用途 |
|---------|------|
| `context7` | ライブラリの最新ドキュメント・コード例の取得（常時有効） |
| `code-review-graph` | コードベースの構造グラフによるトークン効率レビュー（常時有効） |
