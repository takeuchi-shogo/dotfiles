...
-- SF Symbols を使うアプリのリスト
local sf_symbol_apps = { ["Music"] = true }

front_app_icon:subscribe("front_app_switched", function(env)
	local icon_name = env.INFO
	local lookup = app_icons[icon_name]
	local icon = ((lookup == nil) and app_icons["default"] or lookup)
	local font = "sketchybar-app-font-bg:Regular:21.0"
	if sf_symbol_apps[icon_name] then
		font = "SF Pro:Regular:18.0"
	end
	front_app_icon:set({ label = { string = icon, color = colors.accent1, font = font } })
	-- front_app:set(
	--    { label = {
	--      -- string = icon_name,
	--      color = colors.accent1,
